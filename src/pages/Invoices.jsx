...
const Invoices = ({ currentUser, hasPermission, toast }) => {
    ...
    const renderInvoiceRows = (rootRows, pager, opts = {}) => {
        ...
        if (rootRows.length === 0) {
            return <tr><td colSpan={colSpan} className="px-4 py-10 text-center text-slate-400">{emptyText}</td></tr>;
        }
            ...
            rows.push(
                <tr key={inv.id} onClick={() => openDetail(inv)} className={TROW_CLS + ' ' + rowBg + ' cursor-pointer'}>
                    ...
                    <td className="px-4 py-3">
                        ...
                        <div className="text-[10px] text-slate-400 truncate max-w-[220px]" title={'NPWP: ' + (inv.dealer_npwp || '-')}>NPWP: {inv.dealer_npwp || '-'}</div>
                    </td>
                        ...
                        {(() => {
                            ...
                            return (
                                <div className="flex flex-wrap items-center gap-0.5 mt-0.5" title={`Riwayat reject: ${chain.map(c => `#${c.id}${c.no_invoice ? ' (' + c.no_invoice + ')' : ''}`).join(' → ')}`}>
                                    <span className="text-[8px] font-bold uppercase tracking-wider text-slate-400 mr-0.5">Rwy:</span>
                                    {chain.map((c, idx) => {
                ...
                kids.forEach((child) => {
                    rows.push(
                        <tr key={'child-' + child.id} onClick={() => openDetail(child)} className={TROW_CLS + ' bg-indigo-50/40 dark:bg-indigo-500/5 cursor-pointer'}>
                            ...
                            <td className="px-4 py-3">
                                <div className="font-semibold text-slate-700 dark:text-slate-200 truncate max-w-[220px] pl-5" title={child.dealer_name || '-'}>{child.dealer_name || '-'}</div>
                                <div className="text-[10px] text-slate-400 truncate max-w-[220px]" title={'NPWP: ' + (child.dealer_npwp || '-')}>NPWP: {child.dealer_npwp || '-'}</div>
                            </td>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm">
                            ...
                            {dashStatusDist.length === 0 ? (
                                <div className="py-10 text-center text-sm text-slate-400">Belum ada data</div>
                            ) : (
                                <>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
                    <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <h3 className="font-bold text-slate-800 dark:text-white">Daftar Invoice</h3>
                                <div className="text-[11px] text-slate-400">{filteredInvoices.length} data</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <div key={inv.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 min-w-[200px] max-w-full hover:border-indigo-200 dark:hover:border-indigo-500/40 transition-colors">
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate" title={inv.dealer_name}>#{inv.id} • {inv.dealer_name}</span>
                                                </div>
                                            ...
                                            <div className="text-right shrink-0">
                                                <div className="text-[9px] text-slate-400 uppercase tracking-wider whitespace-nowrap">Total</div>
                                                <div className="text-sm font-black text-slate-800 dark:text-white tabular-nums whitespace-nowrap">{formatCurrency(inv.total_invoice)}</div>
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        <div className="overflow-auto max-h-[600px] custom-scrollbar">
                            <table className="w-full text-sm border-collapse">
                                ...
                                <tbody>
                                    ...
                                    {filteredDealers.length === 0 && <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-400">Belum ada dealer</td></tr>}
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div className="overflow-auto max-h-[600px] custom-scrollbar">
                            <table className="w-full text-sm border-collapse">
                                ...
                                <tbody>
                                    ...
                                    {filteredBarang.length === 0 && <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-400">Belum ada barang</td></tr>}
                                </tbody>
                            </table>
                        </div>
            ...
            {tab === 'rule' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 space-y-3 h-fit">
                        ...
                        <p className="text-[10px] text-slate-400">Default semua akses aktif untuk semua user. Buat rule untuk membatasi akses user/role/divisi tertentu (flag rule akan menimpa default). Target dipilih dari master data.</p>
                    </div>
                    )}
                        ...
                        <div className="overflow-auto max-h-[600px] custom-scrollbar">
                            <table className="w-full text-sm border-collapse">
                                ...
                                <tbody>
                                        ...
                                        <tr key={r.id} className={`${TROW_CLS} ${ri % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-slate-50/70 dark:bg-slate-800/30'}`}>
                                            ...
                                            {PERM_VIEW_FIELDS.map(([key]) => (
                                                <td key={key} className="px-4 py-3 text-center">
                                                    {r[key] ? <CheckCircle2 size={15} className="inline text-emerald-500" /> : <XCircle size={15} className="inline text-slate-300 dark:text-slate-600" />}
                                                </td>
                                            ))}
                                    ...
                                    {filteredRules.length === 0 && <tr><td colSpan={ALL_PERM_FIELDS.length + 3} className="px-4 py-10 text-center text-slate-400">Belum ada rule</td></tr>}
                                </tbody>
                            </table>
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-slate-800 dark:text-white">Alur Approval & Notifikasi Invoice</h3>
                                <p className="text-xs text-slate-400">Konfigurasi urutan tahap, penanggung jawab, dan notifikasi email per tahap alur</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            {mailStatus.loaded && (
                    ...
                    {flowOpen && (
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border-2 border-indigo-500/30 dark:border-indigo-500/40 space-y-4 shadow-xl">
                            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                                ...
                                <button onClick={() => setFlowOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                                    <X size={16} />
                                </button>
                            </div>
                            ...
                            <div className="flex items-center gap-2 overflow-x-auto pb-4 custom-scrollbar">
                                {[...flowSteps].sort((a, b) => a.step_no - b.step_no).map((s, idx, arr) => (
                                    <div key={s.id} className="flex items-center gap-2 shrink-0">
                                        <div className={`p-4 rounded-2xl border ${s.is_active ? 'bg-slate-50 dark:bg-slate-800/80 border-indigo-200 dark:border-indigo-500/30' : 'bg-slate-100/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 opacity-60'} min-w-[200px] space-y-2 relative group`}>
                                        ...
                                        {idx < arr.length - 1 && (
                                            <ArrowRight size={18} className="text-slate-300 dark:text-slate-700 shrink-0" />
                                        )}
                                                                    </div>
                                                                ))}
                                                            </div>
                                                    ...
                                                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 space-y-4 shadow-sm">
                                                        <div className="flex flex-wrap items-center justify-between gap-3">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div>
                                                                    <h3 className="font-bold text-slate-800 dark:text-white">Template Email Notifikasi</h3>
                                                                    <p className="text-xs text-slate-400">Kustomisasi subjek & isi email per event. Gunakan token agar data terisi otomatis.</p>
                                                                </div>
                                                            </div>
                                                            {emailTpl.loaded && (
                                                        ...
                                                        {!emailTpl.loaded ? (
                                                            <div className="text-center text-sm text-slate-400 py-6">Memuat template...</div>
                                                        ) : (
                        ...
                        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setRecipOpen(false)}>
                            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
                                ...
                                <div className="p-5 overflow-y-auto custom-scrollbar flex-1">
                                    {recipLoading ? (
                                        <div className="text-center text-sm text-slate-400 py-8">Memuat penerima...</div>
                                    ) : recipList.length === 0 ? (
                                        <div className="text-center text-sm text-slate-400 py-8 space-y-1">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-5xl my-8 p-6 space-y-5" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div className="text-xs text-slate-500 space-x-3">
                                <span>Total Proforma: <b className="text-slate-800 dark:text-white">{formatCurrency(round2((settleTarget?.invoices || []).filter(i => !(i.pp_type === 'pelunasan')).reduce((s, i) => s + (parseFloat(i.total_invoice) || 0), 0)))}</b></span>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-4xl my-8 p-6 space-y-5" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 items-stretch">
                            ...
                            <div className="flex flex-col gap-1.5 min-w-0">
                                ...
                                <div className="h-[12px] text-[9px] text-slate-400 truncate" title="Otomatis dari barang, bisa disesuaikan jika ada selisih pembulatan">Auto dari barang • bisa disesuaikan (pembulatan)</div>
                            </div>
                            {invForm.tipe === 'PP' && (() => {
                                ...
                                return (
                                    <div className="flex flex-col gap-1.5 min-w-0">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase truncate">Sisa (Yang Harus Dilunasi)</label>
                                        ...
                                        <div className="h-[12px] text-[9px] text-slate-400 truncate" title={sisaLabel}>{sisaLabel}</div>
                                    </div>
                                );
                            })()}
                        </div>
                    ...
                    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4" onClick={() => setCancelTarget(null)}>
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-base font-black text-slate-800 dark:text-white leading-tight">Konfirmasi Pembatalan Invoice</h3>
                                    <p className="text-xs text-slate-400 mt-0.5">Invoice #{cancelTarget.id} • {cancelTarget.dealer_name || '-'}</p>
                                </div>
                            </div>
                    ...
                    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4" onClick={() => setDeleteReplTarget(null)}>
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-base font-black text-slate-800 dark:text-white leading-tight">Hapus Invoice Pengganti?</h3>
                                    <p className="text-xs text-slate-400 mt-0.5">Invoice #{deleteReplTarget.id} • {deleteReplTarget.no_invoice || '-'} • {deleteReplTarget.dealer_name || '-'}</p>
                                </div>
                            </div>
                ...
                >
                    ...
                    >
                        {(() => {
                            ...
                            }[type] || { title: type, icon: <CheckCircle2 size={20} className="text-slate-600" />, accent: 'bg-slate-500', confirm: 'Simpan', noteRequired: false, noteLabel: '', notePlaceholder: '' };
                                ...
                                <>
                                    ...
                                    <div className="px-6 py-5">
                                        ...
                                        <div className="mt-5 space-y-2.5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 p-4 text-sm">
                                            ...
                                            <div className="flex justify-between gap-3"><span className="text-xs text-slate-400 shrink-0">Total</span><span className="font-black text-slate-800 dark:text-white tabular-nums text-right">{formatCurrency(total)}</span></div>
                                            <div className="flex justify-between gap-3"><span className="text-xs text-slate-400 shrink-0">Diajukan oleh</span><span className="font-semibold text-slate-700 dark:text-slate-200 text-right">{p?.requested_by || (p?._proforma?.requested_by) || '-'}</span></div>
                                            {invCount > 1 && <div className="flex justify-between gap-3"><span className="text-xs text-slate-400 shrink-0">Invoice</span><span className="font-semibold text-slate-700 dark:text-slate-200 text-right">{invCount} invoice</span></div>}
                                        </div>
