...
export default function MasterData({
    ...
    const text = isEnglish
        ? {
            ...
            autoDetect: 'Auto-detect Fastest',
            useThis: 'Use',
            modelsWorking: 'models working',
            autoFallback: 'Auto Fallback',
            modelNotInList: 'Model "{model}" is not in the gateway list — chat will fail. Pick from the list or run Verify Models.',
            fallbackInfo: 'Auto-fallback: if the main model fails, the AI Agent automatically tries the backup models.',
            savedAsFallback: 'saved as auto fallback',
            aiStatus: 'Status',
        ...
        : {
            ...
            save: 'Simpan',
            verifyModels: 'Verifikasi Model',
            autoDetect: 'Auto-detect Tercepat',
            useThis: 'Pilih',
            modelsWorking: 'model berfungsi',
            autoFallback: 'Fallback Otomatis',
            modelNotInList: 'Model "{model}" tidak ada di daftar gateway — chat akan gagal. Pilih model dari daftar atau klik Verifikasi Model.',
            fallbackInfo: 'Auto-fallback: jika model utama gagal, AI Agent otomatis mencoba model cadangan.',
            savedAsFallback: 'disimpan sebagai fallback otomatis',
            aiStatus: 'Status',
            departmentList: 'Daftar Departemen',
            newDepartment: 'Departemen Baru',
            trainingTitle: 'Dokumen Training AI',
    ...
    const [aiVerifyResults, setAiVerifyResults] = useState(null);
    ...
    const providerOf = (m) => {
        const s = String(m || '');
        const i = s.indexOf('/');
        return i > 0 ? s.slice(0, i) : 'default';
    };
    const providerChipClass = (p) => {
        const palette = [
    ...
    const groupedAiModels = useMemo(() => {
        const map = {};
        for (const m of aiModels) { const p = providerOf(m); (map[p] = map[p] || []).push(m); }
        return Object.entries(map).map(([provider, items]) => ({ provider, items })).sort((a, b) => a.provider.localeCompare(b.provider));
    }, [aiModels]);
    const currentModelInList = !modelsLoading && aiModels.length > 0 && aiForm.model ? aiModels.includes(aiForm.model) : true;

    const verifyAiModels = async () => {
        setAiVerifying(true);
        setAiMsg(null);
        setAiVerifyResults(null);
        try {
            const d = await apiClient.fetchJson(`${API_URL}/settings/ai/models/verify`, {
                method: 'POST',
                body: JSON.stringify({ base_url: aiForm.base_url, api_key: aiForm.api_key, model: aiForm.model }),
            });
            setAiVerifyResults(d);
            if (d.worked > 0) {
                setAiMsg({ type: 'success', text: `${d.worked}/${d.total} ${text.modelsWorking} — ${d.fallbackSaved.length} ${text.savedAsFallback}${d.fromCache ? ' (cache 5 mnt)' : ''}` });
            } else {
                setAiMsg({ type: 'error', text: `Tidak ada model yang berfungsi (${d.total} diuji). Periksa channel di gateway LLM.` });
            }
        } catch (e) {
            setAiMsg({ type: 'error', text: 'Verifikasi gagal: ' + e.message });
        } finally {
            setAiVerifying(false);
        }
    };

    // ── Auto-detect: benchmark semua model, pilih tercepat sebagai utama + 3 cadangan ──
    const autoDetectFastest = async () => {
        setAiVerifying(true);
        setAiMsg(null);
        setAiVerifyResults(null);
        try {
            const d = await apiClient.fetchJson(`${API_URL}/settings/ai/models/verify`, {
                method: 'POST',
                body: JSON.stringify({ base_url: aiForm.base_url, api_key: aiForm.api_key, model: aiForm.model, apply_fastest: true }),
            });
            setAiVerifyResults(d);
            if (d.appliedModel) {
                const fastestMs = (d.results || []).find(r => r.model === d.appliedModel)?.ms;
                setAiForm(f => ({ ...f, model: d.appliedModel }));
                const fallbackTxt = (d.appliedFallbacks || []).join(', ') || '-';
                setAiMsg({
                    type: 'success',
                    text: `⚡ ${text.autoDetect}: ${d.appliedModel} (${fastestMs != null ? fastestMs + 'ms' : 'tercepat'}) — fallback: ${fallbackTxt}`,
                });
            } else if (d.worked > 0) {
                setAiMsg({ type: 'error', text: 'Tidak ada model yang mendukung tool-calling. Coba model lain di gateway.' });
            } else {
                setAiMsg({ type: 'error', text: `Tidak ada model yang berfungsi (${d.total} diuji). Periksa channel di gateway LLM.` });
            }
        } catch (e) {
            setAiMsg({ type: 'error', text: 'Auto-detect gagal: ' + e.message });
        } finally {
            setAiVerifying(false);
        }
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex bg-white/60 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border border-white/40 dark:border-white/10 p-1 rounded-xl w-fit mb-4 shadow-sm">
                            ...
                            Object.entries(groupedUsers).map(([deptName, deptUsers]) => (
                                <div key={deptName} className="space-y-2">
                                    <button
                                        ...
                                        className="w-full flex items-center justify-between p-4 bg-white/70 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-2xl hover:shadow-md transition-all group"
                                    >
                                    ...
                                    {expandedDepts[deptName] && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-4 animate-in slide-in-from-top-2 duration-300">
                                            {deptUsers.map(u => (
                                                <div key={u.id} className="flex items-center justify-between p-4 bg-white/40 dark:bg-[#0d0d0d]/40 backdrop-blur-sm rounded-2xl border border-white/60 dark:border-white/5 hover:border-blue-300 transition-all group/user">
                                                    <div className="flex items-center gap-4">
                                                        <div className="relative">
                                                            <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-blue-500 to-blue-500 flex items-center justify-center text-white font-black text-lg shadow-lg">
                                                                {u.name.charAt(0)}
                                                            </div>
                                                            <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-white/70 dark:bg-[#0d0d0d]/60 rounded-lg flex items-center justify-center border border-white/60 dark:border-white/10 shadow-sm">
                ...
                <Card>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {roles.map(r => {
                            ...
                            return (
                                <div key={r.id} className="p-4 bg-white/70 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-xl">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {/* All known modules — including ones with no rule yet (e.g. new menus like AnyDoc) */}
                                        {Object.values(APP_MODULES).map(mod => {
                                            const actions = perms[mod.id];
                                            const hasRule = Array.isArray(actions) && actions.length > 0;
                                            return (
                                                <button
                                                    key={mod.id}
                                                    onClick={() => handleEditRole(r)}
                                                    aria-disabled={!hasPermission('master', 'edit')}
                                        ...
                                        {/* Unknown modules stored in DB but not in APP_MODULES */}
                                        {Object.keys(perms)
            ...
            {masterTab === 'flows' && (
                <Card>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {flows.map(f => (
                            <div key={f.id} className="p-4 bg-white/70 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-xl hover:border-blue-300 transition-all">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                ...
                                <div className="space-y-2">
                                    {(f.steps || []).map((s, idx) => (
                                        <div key={idx} className="flex items-center gap-2 text-xs">
                                            ...
                                            <span className="dark:text-white/70 font-medium flex items-center gap-1"><ShieldCheck size={12} /> {s.name}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <Card>
                        ...
                        <div className="space-y-4 max-w-2xl">
                            ...
                            <div>
                                ...
                                {!currentModelInList && (
                                    <div className="mt-2 flex items-start gap-2 text-[11px] px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700/40">
                                        ...
                                        <span>{text.modelNotInList.replace('{model}', aiForm.model)}</span>
                                    </div>
                                )}
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                {hasPermission('master', 'edit') && (
                                    <button
                                        onClick={verifyAiModels} disabled={aiVerifying}
                                        className="px-4 py-2 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 rounded-lg text-sm flex items-center gap-2 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-colors disabled:opacity-50"
                                    >
                                        {aiVerifying ? <Loader2 size={16} className="animate-spin" /> : <ShieldCheck size={16} />} {text.verifyModels}
                                    </button>
                                )}
                                {hasPermission('master', 'edit') && (
                                    <button
                                        onClick={autoDetectFastest} disabled={aiVerifying}
                                        title={isEnglish ? 'Benchmark models and auto-select the fastest with 3 fallbacks' : 'Benchmark model & pilih otomatis yang tercepat + 3 cadangan'}
                                        className="px-4 py-2 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 rounded-lg text-sm flex items-center gap-2 hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors disabled:opacity-50"
                                    >
                                        {aiVerifying ? <Loader2 size={16} className="animate-spin" /> : <Gauge size={16} />} {text.autoDetect}
                                    </button>
                                )}
                            </div>
                                ...
                                <div className="rounded-xl border border-white/60 dark:border-white/10 bg-slate-50 dark:bg-[#0d0d0d]/40 p-3">
                                    ...
                                    <div className="flex flex-wrap gap-1.5 max-h-44 overflow-y-auto pr-1">
                                        {aiVerifyResults.results.map(r => (
                                            <div key={r.model} title={r.error || r.model} className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border text-[10px] font-bold ${r.ok ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300' : 'bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-800 text-red-600 dark:text-red-300 opacity-70'}`}>
                    ...
                    <Card>
                        ...
                        {trainingTab === 'learning' && (
                            <div className="space-y-6">
                                ...
                                {learningStats && (
                                    <SummaryRow cards={[
                                        { title: 'Knowledge Points', value: learningStats.totalKnowledgePoints, icon: Brain, gradient: 'from-blue-500 to-blue-600', valueClass: 'text-xl' },
                                        ...
                                        { title: 'Docs Generated', value: learningStats.docsGenerated, icon: FileText, gradient: 'from-blue-500 to-blue-600', valueClass: 'text-xl' },
                                    ]} />
                                )}
                                ...
                                {learningLogs.length > 0 && (
                                    <div>
                                        ...
                                        <div className="border dark:border-white/[0.06]/50 rounded-lg overflow-hidden mt-2">
                                ...
                                {correctionStats && (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        ...
                                        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center">
                                            <div className="text-2xl font-black text-blue-600 dark:text-blue-300">{correctionStats.byType?.length || 0}</div>
                                            <div className="text-xs text-blue-500 dark:text-blue-400 mt-1">Types</div>
                                        </div>
                                    </div>
                                )}
                            ...
                            <div className="space-y-6">
                                ...
                                {evolutionStats && (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        ...
                                        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center">
                                            <div className="text-2xl font-black text-blue-600 dark:text-blue-300">{evolutionStats.latestEvolution?.docs_scanned || 0}</div>
                                            <div className="text-xs text-blue-500 dark:text-blue-400 mt-1">Docs Scanned</div>
                                        </div>
                                    </div>
                                )}
                        ...
                        {trainingTab === 'brain' && (
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    ...
                                    <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center">
                                        <div className="text-2xl font-black text-blue-600 dark:text-blue-300">{brainStats?.associationCount ?? '-'}</div>
                                        <div className="text-xs text-blue-500 dark:text-blue-400 mt-1">Associations</div>
                                    </div>
                                    <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 text-center">
                                ...
                                <div className="bg-white/70 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border dark:border-white/[0.06]/50 rounded-lg p-3">
                                    ...
                                    <div className="flex gap-2">
                                        <Input
                                            ...
                                            className="flex-1 px-3 py-2 border rounded-lg dark:bg-[#0d0d0d] dark:border-white/[0.06] dark:text-white text-sm"
                                ...
                                <div className="bg-white/70 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border dark:border-white/[0.06]/50 rounded-lg p-3">
                                    ...
                                    {brainSyncResult && (
                                        <div className="mt-2 text-xs bg-emerald-50 dark:bg-emerald-900/20 rounded p-2 text-emerald-700 dark:text-emerald-300">
                                            ✅ {brainSyncResult.synced} dokumen tersinkron · 🧠 {brainSyncResult.totalStored} chunk memori{brainSyncResult.skipped ? ` · ⏭ ${brainSyncResult.skipped} dilewati` : ''} · ❌ {brainSyncResult.errors} gagal
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="bg-white/70 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border dark:border-white/[0.06]/50 rounded-lg p-3">
                                    ...
                                    <form onSubmit={handleBrainIngest} className="space-y-2">
                                        <Input
                                            type="text"
                                            placeholder="Judul..."
                                            className="w-full px-3 py-2 border rounded-lg dark:bg-[#0d0d0d] dark:border-white/[0.06] dark:text-white text-sm"
            ...
            {trainingPreview && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setTrainingPreview(null)}>
                    <div className="bg-white/95 dark:bg-[#0d0d0d]/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col border dark:border-white/[0.06]/50" onClick={e => e.stopPropagation()}>
            ...
            {trainingDetail && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setTrainingDetail(null)}>
                    <div className="bg-white/95 dark:bg-[#0d0d0d]/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col border dark:border-white/[0.06]/50" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-5 border-b dark:border-white/[0.06]/50">
                            <div className="flex items-center gap-2">
