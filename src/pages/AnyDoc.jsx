            ...
            {lines.map((line, i) => {
                ...
                if (trimmed.startsWith('|') || trimmed.endsWith('|')) {
                    ...
                    if (cells.length > 0) {
                        ...
                        return (
                            <div key={i} className={`grid gap-2 py-1 px-2 text-xs border-b ${isDarkMode ? 'border-white/5' : 'border-stone-100'}`}
                ...
                if (h1) return <div key={i} className={`text-lg font-extrabold mt-3 ${isDarkMode ? 'text-white' : 'text-stone-800'}`}>{h1[1]}</div>;
                ...
                if (ul) return <div key={i} className={`flex gap-2 text-sm ${isDarkMode ? 'text-white/80' : 'text-stone-700'}`}><span className="text-blue-500">•</span><span>{ul[1]}</span></div>;
                if (ol) return <div key={i} className={`flex gap-2 text-sm ${isDarkMode ? 'text-white/80' : 'text-stone-700'}`}><span className="text-blue-500 font-bold">{ol[1]}.</span><span>{ol[2]}</span></div>;

                // Blockquote
                if (trimmed.startsWith('>')) return <div key={i} className={`text-sm italic pl-3 border-l-4 ${isDarkMode ? 'border-blue-500/50 text-white/60' : 'border-blue-300 text-stone-500'}`}>{trimmed.replace(/^>\s?/, '')}</div>;
                ...
                return (
                    <p key={i} className={`text-sm leading-relaxed ${isDarkMode ? 'text-white/85' : 'text-stone-700'}`}>
                        {parts.map((p, pi) => {
                            if (p.startsWith('**') && p.endsWith('**')) return <strong key={pi} className={`font-bold ${isDarkMode ? 'text-white' : 'text-stone-900'}`}>{p.slice(2, -2)}</strong>;
                            if (p.startsWith('*') && p.endsWith('*') && p.length > 2) return <em key={pi} className="italic">{p.slice(1, -1)}</em>;
                            if (p.startsWith('`') && p.endsWith('`')) return <code key={pi} className={`px-1 py-0.5 rounded text-xs font-mono ${isDarkMode ? 'bg-white/10 text-pink-300' : 'bg-stone-100 text-pink-600'}`}>{p.slice(1, -1)}</code>;
                            return p;
                        })}
                    </p>
                );
    ...
    return (
        <div>
            <div className="flex flex-wrap gap-1.5 mb-2">
                ...
                {items.length === 0 && (
                    <span className={`text-[11px] ${isDarkMode ? 'text-white/30' : 'text-stone-300'}`}>Belum ada kolom.</span>
                )}
            </div>
            <div className="flex gap-1.5">
                <input
                    ...
                    className={`flex-1 min-w-0 px-2.5 py-1.5 rounded-lg text-xs border outline-none transition-colors ${isDarkMode
                        ...
                        : 'bg-white/70 backdrop-blur-xl border-stone-200 text-stone-700 placeholder-slate-300 focus:border-blue-400'}`}
                />
...
export default function AnyDoc({ isDarkMode, currentUser }) {
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto">
            ...
            <PageHeader
                ...
                meta={(
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={`text-[10px] ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>≈ 5 ms rata-rata per dokumen</span>
                    </div>
                )}
                actions={(
                    <button
                        ...
                        className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all border ${isDarkMode
                            ...
                            : 'bg-white/70 backdrop-blur-xl border-stone-200 text-stone-600 hover:bg-stone-50 shadow-sm'}`}
                    >
            ...
            {/* Mode toggle */}
            <div className={`inline-flex p-1 rounded-xl border gap-1 mb-5 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                <button
            ...
            {/* History drawer */}
            {showHistory && (
                <div className={`overflow-hidden mb-4 rounded-2xl border p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                    <div className="flex items-center justify-between mb-3">
                        <h3 className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-stone-700'}`}>Hasil Konversi Terbaru</h3>
                        <button onClick={() => setShowHistory(false)} className={`p-1 rounded-lg ${isDarkMode ? 'hover:bg-white/10 text-white/50' : 'hover:bg-stone-100 text-stone-400'}`}><X size={15} /></button>
                    </div>
                    {history.length === 0 ? (
                        <p className={`text-xs text-center py-4 ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>Belum ada hasil konversi.</p>
                    ) : (
                        <div className="grid gap-2 md:grid-cols-2">
                            {history.map((h, i) => (
                                <a key={i} href={h.url} target="_blank" rel="noreferrer"
                                    className={`flex items-center gap-3 p-3 rounded-xl border transition-all hover:scale-[1.01] ${isDarkMode ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-stone-50 border-stone-200 hover:bg-stone-100'}`}>
                                    <div className={`w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center flex-shrink-0`}>
                                        <FileText size={14} className="text-white" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-xs font-semibold truncate ${isDarkMode ? 'text-white' : 'text-stone-700'}`}>{h.name}</p>
                                        <p className={`text-[10px] flex items-center gap-1 ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>
                                            <Clock size={9} /> {formatRelativeTime(h.modifiedAt)} • {formatFileSize(h.size)}
                                            {h.created_by ? ` • oleh ${h.created_by}` : ''}
                                        </p>
                                    </div>
                                    <Download size={14} className={`flex-shrink-0 ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`} />
                                </a>
                            ))}
                        </div>
                    )}
            ...
            <div className="grid lg:grid-cols-2 gap-6 items-start">
                ...
                <div className="space-y-4">
                    ...
                    <div
                        ...
                        className={`relative rounded-2xl border-2 border-dashed p-8 text-center cursor-pointer transition-all group ${dragOver
                            ...
                            : isDarkMode
                                ...
                                : 'border-stone-300 bg-white hover:border-blue-400 hover:bg-blue-50/40 shadow-sm'}`}
                    >
                        <input
                            ref={inputRef}
                            type="file"
                        ...
                        <p className={`text-xs mt-1 ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>
                    ...
                    {/* Supported formats */}
                    <div className={`rounded-2xl border p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                        <p className={`text-[10px] font-black uppercase tracking-widest mb-3 ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>Format Didukung</p>
                        <div className="grid grid-cols-2 gap-2">
                            {SUPPORTED_FORMATS.map((f, i) => (
                                <div key={i} className={`flex items-center gap-2.5 p-2.5 rounded-xl border ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-stone-50 border-stone-100'}`}>
                                    ...
                                    <div className="min-w-0">
                                        <p className={`text-xs font-bold ${isDarkMode ? 'text-white/80' : 'text-stone-700'}`}>{f.ext}</p>
                                        <p className={`text-[9px] truncate ${isDarkMode ? 'text-white/35' : 'text-stone-400'}`}>{f.desc}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="space-y-4">
                    {!result ? (
                        <div className={`rounded-2xl border-2 border-dashed p-10 text-center min-h-[300px] flex flex-col items-center justify-center ${isDarkMode ? 'border-white/10 bg-white/2' : 'border-stone-200 bg-white/50'}`}>
                            ...
                            <p className={`text-sm font-bold ${isDarkMode ? 'text-white/60' : 'text-stone-500'}`}>Hasil Markdown akan tampil di sini</p>
                        ...
                        <>
                            ...
                            <div className={`rounded-2xl border p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                                <div className="flex items-center gap-3 flex-wrap">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-sm font-bold truncate ${isDarkMode ? 'text-white' : 'text-stone-700'}`}>{result.originalName}</p>
                                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                            ...
                                            <span className={`text-[10px] ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>{result.charCount} karakter • {result.lineCount} baris</span>
                                        </div>
                                    </div>
                                    <button onClick={resetAll} className={`p-2 rounded-lg ${isDarkMode ? 'hover:bg-white/10 text-white/40' : 'hover:bg-stone-100 text-stone-400'}`} title="Reset">
                                        <Trash2 size={15} />
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3">
                                    <button onClick={handleCopy}
                                        className={`flex items-center justify-center gap-1.5 py-2 rounded-xl text-[11px] font-bold transition-all border ${isDarkMode
                                            ? 'bg-white/5 border-white/10 text-white/70 hover:bg-white/10'
                                            : 'bg-white/70 backdrop-blur-xl border-stone-200 text-stone-600 hover:bg-stone-50'}`}>
                            ...
                            <div className={`rounded-2xl border overflow-hidden ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                                <div className={`flex items-center gap-2 px-4 py-2.5 border-b ${isDarkMode ? 'border-white/10' : 'border-stone-100'}`}>
                                    <div className="flex gap-1.5">
                                        <span className="w-2.5 h-2.5 rounded-full bg-rose-400" />
                                        <span className="w-2.5 h-2.5 rounded-full bg-amber-400" />
                                        <span className="w-2.5 h-2.5 rounded-full bg-emerald-400" />
                                    </div>
                                    <span className={`text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 ml-2 ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>
                            ...
                            {/* Raw toggle */}
                            <details className={`rounded-2xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200'}`}>
                                <summary className={`px-4 py-2.5 text-[11px] font-bold cursor-pointer select-none flex items-center gap-1.5 ${isDarkMode ? 'text-white/50 hover:text-white/80' : 'text-stone-400 hover:text-stone-600'}`}>
                                    <FileCode2 size={13} /> Lihat kode Markdown mentah
                                </summary>
                                <pre className={`p-4 text-xs font-mono overflow-x-auto border-t ${isDarkMode ? 'bg-black/30 border-white/10 text-emerald-300/90' : 'bg-stone-50 border-stone-100 text-stone-600'}`}>
                                    {result.markdown}
                                </pre>
                            </details>
                        </>
                ...
                <>
                    ...
                    <div className="grid lg:grid-cols-2 gap-6 items-start">
                        ...
                        <div className="space-y-4">
                            <div
                                ...
                                className={`relative rounded-2xl border-2 border-dashed p-7 text-center cursor-pointer transition-all ${dragOver2
                                    ...
                                    : isDarkMode
                                        ...
                                        : 'border-stone-300 bg-white hover:border-emerald-400 hover:bg-emerald-50/40 shadow-sm'}`}
                            >
                                <input
                                    ref={extractInputRef}
                                    type="file"
                            ...
                            {extractFiles.length > 0 && (
                                <div className={`rounded-2xl border p-3 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                                    <div className="space-y-1.5">
                                        {extractFiles.map((f, i) => (
                                            <div key={i} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg ${isDarkMode ? 'bg-white/5' : 'bg-stone-50'}`}>
                                                ...
                                                <button onClick={() => removeExtractFile(i)} className={`p-1 rounded-md flex-shrink-0 ${isDarkMode ? 'hover:bg-white/10 text-white/40' : 'hover:bg-stone-200 text-stone-400'}`} title="Hapus"><X size={12} /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className={`rounded-2xl border p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                                <p className={`text-[10px] font-black uppercase tracking-widest mb-3 ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>Mapping Kolom</p>
                                ...
                                <p className={`text-[10px] mt-3 leading-relaxed ${isDarkMode ? 'text-white/35' : 'text-stone-400'}`}>
                        ...
                        <div className="space-y-4">
                            {results.length === 0 ? (
                                <div className={`rounded-2xl border-2 border-dashed p-10 text-center min-h-[300px] flex flex-col items-center justify-center ${isDarkMode ? 'border-white/10 bg-white/2' : 'border-stone-200 bg-white/50'}`}>
                                    <div className={`text-5xl mb-4 ${isDarkMode ? 'opacity-30' : 'opacity-20'}`}>📊 → 📑</div>
                                    <p className={`text-sm font-bold ${isDarkMode ? 'text-white/60' : 'text-stone-500'}`}>Hasil ekstraksi akan tampil di sini</p>
                                    <p className={`text-xs mt-1 max-w-xs ${isDarkMode ? 'text-white/30' : 'text-stone-400'}`}>
                                        ...
                                        return (
                                            <div className={`rounded-2xl border p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/70 backdrop-blur-xl border-stone-200 shadow-sm'}`}>
                                                <div className="flex items-center gap-3 flex-wrap">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-stone-700'}`}>Hasil Ekstraksi</p>
                                                        <p className={`text-[11px] mt-0.5 flex items-center gap-2 flex-wrap ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>
                                            ...
                                            <table className="w-full text-left text-xs">
                                                <thead>
                                                    <tr className={`border-b ${isDarkMode ? 'border-white/10' : 'border-stone-100'}`}>
                                                        <th className={`px-3 py-2.5 text-[10px] font-black uppercase tracking-wider sticky left-0 ${isDarkMode ? 'bg-[#161a2e] text-white/40' : 'bg-white text-stone-400'}`}>File</th>
                                                        {fields.map((f, i) => (
                                                            <th key={i} className={`px-3 py-2.5 text-[10px] font-black uppercase tracking-wider whitespace-nowrap ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>{f}</th>
                                                        ))}
                                                        <th className={`px-3 py-2.5 text-[10px] font-black uppercase tracking-wider ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>Status</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {results.map((r, ri) => (
                                                        <React.Fragment key={ri}>
                                                            <tr className={`border-b ${isDarkMode ? 'border-white/5' : 'border-stone-50'}`}>
                                                                <td className={`px-3 py-2 text-[11px] font-bold max-w-[220px] truncate sticky left-0 ${isDarkMode ? 'bg-[#161a2e] text-white/80' : 'bg-white text-stone-700'}`} title={r.filename}>{r.filename}</td>
                                                                {fields.map((f, ci) => (
                                                                        ...
                                                                        {r.success ? (
                                                                            <input
                                                                                ...
                                                                                className={`w-full px-2 py-1 rounded-lg text-[11px] border outline-none transition-colors ${isDarkMode
                                                                                    ...
                                                                                    : 'bg-white/70 backdrop-blur-xl border-stone-200 text-stone-700 focus:border-emerald-400'}`}
                                                                            />
                                                                        ) : (
                                                                            <span className={`text-[11px] ${isDarkMode ? 'text-white/20' : 'text-stone-300'}`}>—</span>
                                                                        )}
                                                                    </td>
                                                                ))}
                                                                    ...
                                                                    <td colSpan={fields.length + 2} className="p-0">
                                                                        ...
                                                                        {expanded[ri] && (
                                                                            <div className="px-3 pb-3 overflow-x-auto">
                                                                                <table className={`w-full text-left text-[11px] rounded-lg overflow-hidden border ${isDarkMode ? 'border-white/10' : 'border-stone-100'}`}>
                                                                                    <thead>
                                                                                        <tr className={isDarkMode ? 'bg-white/5' : 'bg-stone-50'}>
                                                                                            <th className={`px-2.5 py-1.5 text-[10px] font-black uppercase ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>No</th>
                                                                                            {itemFields.map((f, ci) => (
                                                                                                <th key={ci} className={`px-2.5 py-1.5 text-[10px] font-black uppercase whitespace-nowrap ${isDarkMode ? 'text-white/40' : 'text-stone-400'}`}>{f}</th>
                                                                                            ))}
                                                                                        </tr>
                                                                                    </thead>
                                                                                    <tbody>
                                                                                        {(r.items || []).map((it, ii) => (
                                                                                            <tr key={ii} className={`border-t ${isDarkMode ? 'border-white/5' : 'border-stone-50'}`}>
                                                                                                <td className={`px-2.5 py-1.5 ${isDarkMode ? 'text-white/50' : 'text-stone-400'}`}>{ii + 1}</td>
                                                                                                {itemFields.map((f, ci) => (
                                                                                        ...
                                                                                        {(r.items || []).length === 0 && (
                                                                                            <tr className={`border-t ${isDarkMode ? 'border-white/5' : 'border-stone-50'}`}>
                                                                                                <td colSpan={itemFields.length + 1} className={`px-2.5 py-2 ${isDarkMode ? 'text-white/30' : 'text-stone-400'}`}>Tidak ada item barang di dokumen ini.</td>
                                                                                            </tr>
                                                                                        )}
                                                                                    </tbody>
                                                                                </table>
