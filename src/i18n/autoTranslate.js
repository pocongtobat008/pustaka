// ─────────────────────────────────────────────────────────────────────────────
// Mesin terjemahan otomatis id → en untuk teks UI yang belum memakai t().
// Diterapkan oleh <AutoTranslateLayer/> lewat DOM saat bahasa = English.
// Kamus frasa: longest-match dulu, kata per kata sebagai fallback.
// ─────────────────────────────────────────────────────────────────────────────

const ID2EN = {
  // ── Inti UI ──
  'buat': 'create', 'tambah': 'add', 'tambahkan': 'add', 'hapus': 'delete', 'menghapus': 'delete',
  'ubah': 'edit', 'edit': 'edit', 'lihat': 'view', 'melihat': 'view', 'simpan': 'save', 'menyimpan': 'save',
  'batal': 'cancel', 'tutup': 'close', 'keluar': 'logout', 'muat': 'load', 'memuat': 'loading',
  'cari': 'search', 'pencarian': 'search', 'filter': 'filter', 'pilih': 'select', 'memilih': 'select',
  'semua': 'all', 'lainnya': 'others', 'ya': 'yes', 'tidak': 'no', 'lanjut': 'continue', 'kembali': 'back',
  'kirim': 'send', 'mengirim': 'send', 'terima': 'receive', 'unduh': 'download', 'mengunduh': 'downloading',
  'unggah': 'upload', 'mengunggah': 'uploading', 'cetak': 'print', 'ekspor': 'export', 'impor': 'import',
  'terapkan': 'apply', 'konfirmasi': 'confirm', 'perbarui': 'update', 'memperbarui': 'update',
  'proses': 'process', 'memproses': 'processing', 'sedang': 'currently', 'selesai': 'done',
  'sukai': 'like', 'bagikan': 'share', 'berbagi': 'share', 'duplikat': 'duplicate', 'salin': 'copy',
  'tempel': 'paste', 'pindah': 'move', 'ganti': 'change', 'mengganti': 'replace', 'atur': 'arrange',
  'kelola': 'manage', 'pengaturan': 'settings', 'preferensi': 'preferences', 'profil': 'profile',
  'beranda': 'home', 'menu': 'menu', 'item': 'item', 'items': 'items', 'semua menu': 'all menus',
  'modul': 'module', 'grup': 'group', 'kategori': 'category', 'halaman': 'page', 'tab': 'tab',

  // ── Umum / frasa pendek ──
  'berhasil': 'successfully', 'sukses': 'success', 'gagal': 'failed', 'kesalahan': 'error',
  'peringatan': 'warning', 'catatan': 'notes', 'note': 'note', 'alasan': 'reason',  'keterangan': 'remarks',
  'pesan': 'message', 'status': 'status', 'aksi': 'action', 'tindakan': 'action', 'kondisi': 'condition',
  'akun': 'account', 'masuk': 'sign in', 'masuk rp': 'received rp', 'berubah': 'changed',
  'wajib': 'required', 'opsional': 'optional', 'diperlukan': 'required', 'mohon': 'please',
  'silakan': 'please', 'harap': 'please', 'pastikan': 'make sure', 'cek': 'check', 'periksa': 'check',
  'yakin': 'sure', 'ingin': 'want to', 'coba': 'try', 'dicoba': 'tried', 'lakukan': 'do', 'dilakukan': 'done',
  'silahkan': 'please', 'maksimal': 'maximum', 'minimal': 'minimum', 'otomatis': 'automatic',
  'default': 'default', 'bawaan': 'default', 'kosong': 'empty', 'penuh': 'full', 'sebagian': 'partial',
  'total': 'total', 'jumlah': 'count', 'sisa': 'remaining', 'selisih': 'difference', 'rata': 'average',
  'rata-rata': 'average', 'estimasi': 'estimate', 'taksiran': 'estimate', 'perkiraan': 'estimate',
  'peringkat': 'ranking', 'urutan': 'order', 'terbaru': 'latest', 'terlama': 'oldest', 'naik': 'ascending',
  'turun': 'descending', 'tertinggi': 'highest', 'terendah': 'lowest', 'terbesar': 'largest',
  'terkecil': 'smallest', 'banyak': 'many', 'sedikit': 'few', 'cepat': 'fast', 'lambat': 'slow',
  'penting': 'important', 'rahasia': 'confidential', 'pribadi': 'private', 'internal': 'internal',
  'eksternal': 'external', 'lokal': 'local', 'online': 'online', 'aktif': 'active', 'nonaktif': 'inactive',
  'mati': 'disabled', 'hidup': 'enabled', 'terbuka': 'open', 'tertutup': 'closed', 'terkunci': 'locked',
  'aman': 'secure', 'keamanan': 'security', 'privasi': 'privacy', 'diizinkan': 'allowed', 'dilarang': 'forbidden',
  'izin': 'permission', 'hak': 'rights', 'peran': 'role', 'level': 'level', 'tingkat': 'level',
  'pengguna': 'users', 'anggota': 'members', 'administrator': 'administrator', 'divisi': 'division',
  'departemen': 'department', 'bagian': 'section', 'tim': 'team', 'unit': 'unit', 'kantor': 'office',
  'pusat': 'head office', 'cabang': 'branch', 'perusahaan': 'company', 'organisasi': 'organization',
  'bisnis': 'business', 'transaksi': 'transaction', 'keuangan': 'finance', 'akuntansi': 'accounting',
  'kas': 'cash', 'bank': 'bank', 'rekening': 'account', 'anggaran': 'budget', 'pengeluaran': 'expense',
  'pemasukan': 'income', 'pendapatan': 'revenue', 'laba': 'profit', 'rugi': 'loss', 'modal': 'capital',
  'hutang': 'debt', 'piutang': 'receivable', 'kewajiban': 'liability', 'aset': 'asset', 'neraca': 'balance sheet',
  'jurnal': 'journal', 'dibukukan': 'booked', 'pembukuan': 'bookkeeping',  'mata uang': 'currency',
  'tunggakan': 'overdue', 'jatuh tempo': 'due date', 'pelunasan': 'settlement', 'pembayaran': 'payment',
  'uang masuk': 'money received', 'uang keluar2': 'money paid out', 'masuk uang': 'money in',
  'diunggah': 'uploaded', 'diunduh': 'downloaded', 'wajib diunggah': 'must be uploaded',
  'sama dengan': 'same as', 'tidak boleh lebih': 'must not exceed', 'lebih besar': 'greater',
  'lebih kecil': 'smaller', 'menunggu approve': 'pending approval', 'menunggu faktur': 'awaiting tax invoice',
  'menunggu update': 'awaiting update', 'selesai di-settle': 'settled',
  'cicilan': 'installment', 'angsuran': 'installment', 'denda': 'fine', 'bunga': 'interest', 'potongan': 'deduction',

  // ── Domain invoice / proforma / pajak ──
  'daftar': 'list', 'data': 'data', 'invoice': 'invoice', 'faktur': 'invoice', 'proforma': 'proforma',
  'pajak': 'tax', 'dealer': 'dealer', 'barang': 'items', 'jasa': 'services', 'stok': 'stock',
  'persediaan': 'inventory', 'gudang': 'warehouse', 'gudang2': 'warehouse', 'nomor': 'number',
  'nominal': 'amount', 'nilai': 'value', 'harga': 'price', 'harga satuan': 'unit price',  'kuantitas': 'quantity', 'jumlah item': 'item count', 'berat': 'weight', 'volume': 'volume', 'dimensi': 'dimension',
  'uang': 'money', 'uang keluar': 'money out', 'subtotal': 'subtotal',
  'diskon': 'discount', 'materai': 'stamp duty', 'ppn': 'VAT', 'pph': 'income tax', 'kurs': 'exchange rate',
  'biaya': 'cost', 'ongkos': 'cost', 'beban': 'expense', 'lain-lain': 'miscellaneous', 'lainnya2': 'other',
  'dibayar': 'paid', 'belum dibayar': 'unpaid', 'terbayar': 'paid', 'pembayaran diterima': 'payment received',
  'pelanggan': 'customer', 'pemasok': 'supplier', 'vendor': 'vendor', 'penjualan': 'sales', 'pembelian': 'purchase',
  'pesanan': 'order', 'pengiriman': 'delivery', 'penerimaan': 'receipt', 'barang masuk': 'incoming goods',
  'barang keluar': 'outgoing goods', 'surat jalan': 'delivery note', 'resi': 'receipt',
  'pengajuan': 'submission', 'persetujuan': 'approval', 'disetujui': 'approved', 'menyetujui': 'approve',
  'setuju': 'approve', 'tolak': 'reject', 'menolak': 'reject', 'ditolak': 'rejected', 'mengirim balik': 'send back',
  'dikembalikan': 'returned', 'dikirim': 'sent', 'dikirim balik': 'sent back', 'diajukan': 'submitted',
  'menunggu': 'pending', 'pending': 'pending', 'draft': 'draft', 'disimpan': 'saved', 'tersimpan': 'saved',
  'diproses': 'processed', 'diperbarui': 'updated', 'dihapus': 'deleted', 'diarsipkan': 'archived',
  'diarsip': 'archive', 'arsip': 'archive', 'dipindah': 'moved', 'dipulihkan': 'restored', 'dipulihkan2': 'recovered',
  'dibatalkan': 'cancelled', 'pembatalan': 'cancellation', 'batal2': 'void', 'revisi': 'revision',
  'pengganti': 'replacement', 'pelunasan dp': 'DP settlement', 'dp': 'down payment', 'down payment': 'down payment',
  'sisa tagihan': 'outstanding balance', 'tagihan': 'bill', 'tgl': 'date', 'tanggal': 'date',
  'nomor invoice': 'invoice number', 'nomor proforma': 'proforma number', 'nomor po': 'PO number',
  'nomor faktur': 'invoice number', 'no invoice': 'invoice no.', 'no po': 'PO no.', 'no proforma': 'proforma no.',
  'tenggat': 'deadline', 'berlaku': 'valid', 'kedaluwarsa': 'expired', 'masa berlaku': 'validity period',

  // ── Dokumen / AI / file ──
  'dokumen': 'documents', 'berkas': 'files', 'lampiran': 'attachment', 'lampiran2': 'attachments',
  'berkas2': 'file', 'folder': 'folder', 'direktori': 'directory', 'jenis': 'type', 'tipe': 'type',
  'ukuran': 'size', 'format': 'format', 'halaman2': 'pages', 'sampah': 'trash', 'sampah2': 'recycle bin',
  'pulihkan': 'restore', 'pulihkan2': 'restore', 'hapus permanen': 'delete permanently', 'permanen': 'permanent',
  'sementara': 'temporary', 'riwayat': 'history', 'log': 'log', 'jejak': 'trail', 'audit': 'audit',
  'laporan': 'report', 'ringkasan': 'summary', 'grafik': 'chart', 'diagram': 'chart', 'bagan': 'chart',
  'peta': 'map', 'denah': 'floor plan', 'notifikasi': 'notifications', 'kotak masuk': 'inbox',
  'tugas': 'task', 'pekerjaan': 'job', 'jadwal': 'schedule', 'kalender': 'calendar', 'cuti': 'leave',
  'izin2': 'permission', 'hadir': 'present', 'absensi': 'attendance', 'waktu': 'time', 'jam': 'hour',
  'menit': 'minute', 'detik': 'second', 'hari': 'day', 'minggu': 'week', 'bulan': 'month', 'tahun': 'year',
  'kuartal': 'quarter', 'semester': 'semester', 'periode': 'period', 'sekarang': 'now', 'hari ini': 'today',
  'kemarin': 'yesterday', 'besok': 'tomorrow', 'lalu': 'ago', 'selanjutnya': 'next', 'sebelumnya': 'previous',
  'tampilkan': 'show', 'sembunyikan': 'hide', 'tampil': 'shown', 'tersembunyi': 'hidden', 'buka2': 'open',
  'meluncurkan': 'launch', 'pratinjau': 'preview', 'pratinjau2': 'preview', 'contoh': 'sample',
  'template': 'template', 'pola': 'pattern', 'skema': 'schema', 'struktur': 'structure', 'kolom': 'column',
  'baris2': 'row', 'sel': 'cell', 'judul': 'title', 'deskripsi': 'description', 'kata kunci': 'keywords',
  'paragraf': 'paragraph', 'gambar': 'image', 'foto': 'photo', 'video': 'video', 'suara': 'audio',
  'teks': 'text', 'warna': 'color', 'latar': 'background', 'font': 'font', 'huruf': 'letter',
  'cetak2': 'printed', 'kertas': 'paper', 'ukuran kertas': 'paper size', 'margin': 'margin',
  'posisi': 'position', 'lokasi': 'location', 'arah': 'direction', 'kanan': 'right', 'kiri': 'left',
  'atas': 'top', 'bawah': 'bottom', 'tengah': 'center', 'samping': 'side', 'depan': 'front', 'belakang': 'back',
  'dekat': 'near', 'jauh': 'far', 'sejajar': 'aligned', 'rapi': 'neat', 'berantakan': 'messy',
  'jelas': 'clear', 'samar': 'blurry', 'terang': 'bright', 'gelap': 'dark', 'terang2': 'light',

  // ── AI ──
  'kecerdasan': 'intelligence', 'buatan': 'artificial', 'model': 'model', 'asisten': 'assistant',
  'obrolan': 'chat', 'kirim pesan': 'send message', 'tulis pesan': 'type a message', 'jawaban': 'answer',
  'pertanyaan': 'question', 'pikir': 'thinking', 'analisis': 'analysis', 'analisa': 'analysis',
  'pelatihan': 'training', 'melatih': 'train', 'dilatih': 'trained', 'terlatih': 'trained',
  'dokumen pintar': 'smart documents', 'pemindai': 'scanner', 'pindai': 'scan', 'memindai': 'scanning',
  'dipindai': 'scanned', 'hasil': 'result', 'hasil2': 'results', 'ekstrak': 'extract', 'ekstraksi': 'extraction',
  'diekstrak': 'extracted', 'dipetakan': 'mapped', 'pemetaan': 'mapping', 'bidang': 'field', 'label2': 'label',
  'deteksi': 'detection', 'mendeteksi': 'detect', 'terdeteksi': 'detected', 'pengenalan': 'recognition',
  'mengenali': 'recognize', 'dikenali': 'recognized', 'bahasa': 'language', 'terjemahan': 'translation',
  'menerjemahkan': 'translate', 'diterjemahkan': 'translated', 'suara2': 'voice', 'tanda tangan': 'signature',
  'menandatangani': 'sign', 'ttd': 'signature', 'paraf': 'initial', 'stempel': 'stamp', 'barcode': 'barcode',
  'qrcode': 'QR code', 'kerangka': 'framework', 'pengetahuan': 'knowledge', 'memori': 'memory',
  'semantik': 'semantic', 'graf': 'graph', 'simpul': 'node', 'hubungan': 'relationship', 'terhubung': 'connected',

  // ── Umum penuh kalimat pendek ──
  'tidak ada': 'no', 'belum ada': 'no', 'belum': 'not yet', 'ada': 'exists', 'tidak tersedia': 'unavailable',
  'tidak ditemukan': 'not found', 'tidak valid': 'invalid', 'tidak cocok': 'mismatch', 'tidak aktif': 'inactive',
  'belum diisi': 'not filled', 'sedang memuat': 'loading', 'memuat data': 'loading data',
  'gagal memuat': 'failed to load', 'gagal memuat data': 'failed to load data',
  'data berhasil disimpan': 'data saved successfully', 'data berhasil dihapus': 'data deleted successfully',
  'berhasil disimpan': 'saved successfully', 'berhasil dihapus': 'deleted successfully',
  'berhasil diperbarui': 'updated successfully', 'berhasil ditambahkan': 'added successfully',
  'berhasil dikirim': 'sent successfully', 'berhasil diunggah': 'uploaded successfully',
  'gagal menyimpan': 'failed to save', 'gagal menghapus': 'failed to delete', 'gagal mengirim': 'failed to send',
  'terjadi kesalahan': 'an error occurred', 'terjadi masalah': 'something went wrong',
  'ada masalah': 'there is a problem', 'periksa kembali': 'check again', 'coba lagi': 'try again',
  'silahkan coba lagi': 'please try again', 'mohon tunggu': 'please wait', 'harap tunggu': 'please wait',
  'sebentar lagi': 'in a moment', 'akan diarahkan': 'you will be redirected', 'kembali ke': 'back to',
  'pilih file': 'choose file', 'pilih file lampiran': 'choose attachment file', 'tidak ada file': 'no file',
  'tidak ada file dipilih': 'no file selected', 'file dipilih': 'files selected', 'hapus file': 'remove file',
  'lanjutkan': 'continue', 'batalkan': 'cancel', 'simpan perubahan': 'save changes', 'simpan dulu': 'save first',
  'keluar dari': 'sign out of', 'masuk ke': 'sign in to', 'login': 'sign in', 'daftar akun': 'create account',
  'lupa password': 'forgot password', 'kata sandi': 'password', 'nama pengguna': 'username',
  'nama lengkap': 'full name', 'nomor hp': 'phone number', 'nomor telepon': 'phone number',

  // ── Statistik / dashboard ──
  'indikator': 'indicator', 'kinerja': 'performance', 'pencapaian': 'achievement', 'target': 'target',
  'realisasi': 'realization', 'pertumbuhan': 'growth', 'perubahan': 'change', 'tren': 'trend',
  'sebaran': 'distribution', 'distribusi': 'distribution', 'perbandingan': 'comparison',
  'teratas': 'top', 'terbaik': 'best', 'teraktif': 'most active', 'terlaris': 'best seller',
  'paling sering': 'most frequent', 'total nominal': 'total amount', 'jumlah data': 'data count',
  'entri': 'entries', 'entry': 'entry', 'catatan2': 'record', 'rekam': 'record', 'direkam': 'recorded',
  'draft2': 'draft', 'konsep': 'concept', 'aktifitas': 'activity', 'kegiatan': 'activity', 'aktivitas': 'activity',
  'seluruh': 'entire', 'keseluruhan': 'overall', 'keseluruhan2': 'whole', 'gabungan': 'combined',
  'semua departemen': 'all departments', 'lintas': 'cross', 'departemen2': 'department',
  'izin akses': 'access permission', 'grup pengguna': 'user group', 'hak akses': 'access rights',
  'superadmin': 'super admin', 'manajer': 'manager', 'staff': 'staff', 'karyawan': 'employee',
  'diterbitkan': 'issued', 'terbit': 'published', 'tersedia': 'available', 'aktifkan': 'enable',
  'nonaktifkan': 'disable', 'mengaktifkan': 'enable', 'menonaktifkan': 'disable', 'diaktifkan': 'enabled',
  'menyalin': 'copy', 'digabung': 'merged', 'dikelompokkan': 'grouped', 'dipisahkan': 'separated',
  'pisahkan': 'separate', 'pemisah': 'separator', 'dipilih': 'selected', 'ditandai': 'marked',
  'tandai': 'mark', 'tandai2': 'flag', 'bertanda': 'flagged', 'simbol': 'symbol', 'ikon': 'icon',
  'logo': 'logo', 'header': 'header', 'footer': 'footer', 'kop': 'letterhead', 'kertas kop': 'letterhead',
  'paraf2': 'initial', 'inisial': 'initial', 'kota': 'city', 'provinsi': 'province', 'negara': 'country',
  'kode pos': 'postal code', 'kecamatan': 'district', 'kelurahan': 'subdistrict', 'jalan': 'street',
  'rt': 'RT', 'rw': 'RW', 'blok': 'block', 'nomor rumah': 'house number', 'patokan': 'landmark',
  'tenggat waktu': 'deadline', 'segera': 'soon', 'mendesak': 'urgent', 'kritis': 'critical', 'normal': 'normal',
  'rendah': 'low', 'sedang2': 'medium', 'tinggi': 'high', 'prioritas': 'priority', 'urutan prioritas': 'priority order',

  // ── Status & verba turunan umum (di-, me-) ──
  'diproses2': 'processed', 'diteruskan': 'forwarded', 'dialihkan': 'redirected', 'dijadwalkan': 'scheduled',
  'dimulai': 'started', 'mulai': 'start', 'berakhir': 'ended', 'akhiri': 'end', 'selesaikan': 'finish',
  'menyelesaikan': 'finishing', 'dilewati': 'skipped', 'lewati': 'skip', 'lewati2': 'skip',
  'diperlukan2': 'needed', 'membutuhkan': 'requires', 'butuh': 'need', 'memerlukan': 'requires',
  'bisa': 'can', 'dapat': 'can', 'mampu': 'able', 'sudah': 'already', 'sudah2': 'has been',
  'akan': 'will', 'harus': 'must', 'sebaiknya': 'should', 'baiknya': 'better', 'disarankan': 'recommended',
  'rekomendasi': 'recommendation', 'rekomendasikan': 'recommend', 'panduan': 'guide', 'instruksi': 'instructions',
  'manual': 'manual', 'panduan pengguna': 'user guide', 'bantuan': 'help', 'pusat bantuan': 'help center',
  'saran': 'suggestion', 'suggest': 'suggestions', 'masukan': 'input', 'feedback': 'feedback', 'ulasan': 'review',
  'penilaian': 'rating', 'nilai2': 'rating', 'skor': 'score', 'komentar': 'comment', 'diskusi': 'discussion',
  'forum': 'forum', 'komunitas': 'community', 'berita': 'news', 'pengumuman': 'announcement',
  'pembaruan': 'update', 'versi': 'version', 'rilis': 'release', 'terbaru2': 'latest',
  'detail': 'details', 'rincian': 'details', 'lengkap': 'complete', 'lengkapi': 'complete', 'lengkap2': 'full',
  'info': 'info', 'informasi': 'information', 'menginformasikan': 'inform', 'diberitahu': 'notified',
  'konfirmasi2': 'confirmation', 'memastikan': 'ensure', 'kejelasan': 'clarity', 'kepastian': 'certainty',

  // ── Search/empty/tabel ──
  'hasil pencarian': 'search results', 'tidak ada hasil': 'no results', 'tidak ada hasil ditemukan': 'no results found',
  'cari berdasarkan': 'search by', 'cari data': 'search data', 'cari nama': 'search name',
  'ketik': 'type', 'masukkan': 'enter', 'masukan2': 'input', 'input': 'input', 'isi': 'fill',
  'diisi': 'filled', 'contoh2': 'e.g.', 'contohnya': 'for example', 'misalnya': 'for example',
  'atau pilih': 'or choose', 'nama2': 'name', 'ditampilkan': 'displayed', 'menampilkan': 'showing',
  'per halaman': 'per page', 'dari total': 'of total', 'pagination': 'pagination',

  // angka ordinal / bantuan
  'pertama': 'first', 'kedua': 'second', 'ketiga': 'third', 'keempat': 'fourth', 'kelima': 'fifth',
  'terakhir': 'last', 'akhir': 'end', 'awal': 'beginning', 'tengah2': 'middle', 'sementara2': 'while',
  'selama': 'during', 'hingga': 'until', 'sampai': 'until', 'setelah': 'after', 'sebelum': 'before',
  'antara': 'between', 'bersama': 'together', 'tanpa': 'without', 'dalam': 'within', 'keluar dari2': 'out of',
  'masuk ke2': 'into', 'menuju': 'towards', 'kepada': 'to', 'karena': 'because', 'sehingga': 'therefore',
  'maka': 'then', 'selain': 'besides', 'termasuk': 'including', 'terkait': 'related', 'berkaitan': 'related to',
  'berdasarkan': 'based on', 'menurut': 'according to', 'sesuai': 'according to', 'cocok2': 'matches',
  'valid': 'valid', 'benar': 'correct', 'salah': 'wrong', 'kembar': 'duplicate', 'duplikat2': 'duplicate',
  'hampir': 'almost', 'sama': 'same', 'berbeda': 'different', 'beda': 'different', 'serupa': 'similar',
  'mendekati': 'close to', 'jauh2': 'far', 'sekitar': 'around', 'kurang lebih': 'approximately',

  // ── Gelombang 2: kata yang masih sering muncul ──
  'ajukan': 'submit', 'ajukan ulang': 'resubmit', 'aplikasi': 'application', 'asal': 'origin',
  'barangnya': 'the items', 'bawahnya': 'below it', 'bayar': 'pay', 'pembayaran2': 'payment',
  'berbagai': 'various', 'bergantung': 'depends', 'bersih': 'clean', 'campuran': 'mixture',
  'dahulu': 'first', 'dasar': 'basic', 'datamu': 'your data', 'diambil': 'taken', 'dibangun': 'built',
  'dibersihkan': 'cleaned', 'dicari': 'searched', 'dicek': 'checked', 'didukung': 'supported',
  'diedit': 'edited', 'dikonfigurasi': 'configured', 'dikoreksi': 'corrected', 'dilihat': 'viewed',
  'dilunasi': 'paid off', 'diperiksa': 'checked', 'dipertahankan': 'maintained', 'dipinjam': 'borrowed',
  'dipotong': 'deducted', 'disesuaikan': 'adjusted', 'ditambah': 'added', 'ditanyakan': 'asked',
  'diwariskan': 'inherited', 'dokumentasi': 'documentation', 'durasi': 'duration', 'gabung': 'merge',
  'geser': 'drag', 'hitung': 'calculate', 'menghitung': 'calculate', 'hubungi': 'contact',
  'jadi': 'become', 'jarak': 'distance', 'jika': 'if', 'juga': 'also', 'kalkulator': 'calculator',
  'khusus': 'special', 'komponen': 'component', 'kompres': 'compress', 'kompresi': 'compression',
  'kursor': 'cursor', 'kustomisasi': 'customization', 'kustom': 'custom', 'lebar': 'width',
  'lepas': 'release', 'lewat': 'via', 'lingkungan': 'environment', 'macet': 'stuck', 'mana': 'which',
  'manajemen': 'management', 'matikan': 'disable', 'membaca': 'read', 'membangun': 'build',
  'membatalkan': 'cancel', 'membatasi': 'limit', 'memulai': 'start', 'menandai': 'mark',
  'mengarsipkan': 'archive', 'mengatur': 'manage', 'mengekstrak': 'extract', 'mengelola': 'manage',
  'mengelompokkan': 'group', 'menghadapi': 'face', 'mengisi': 'fill', 'mengonversi': 'convert',
  'mengoreksi': 'correct', 'menjelajah': 'browse', 'mentah': 'raw', 'menyesuaikan': 'adjust',
  'mesin': 'machine', 'miring': 'italic', 'namun': 'however', 'opsi': 'option', 'orientasi': 'orientation',
  'parsial': 'partial', 'pecah': 'split', 'pegawai': 'employee', 'pelaporan': 'reporting',
  'pembetulan': 'correction', 'pembulatan': 'rounding', 'pemeriksaan': 'inspection', 'pemicu': 'trigger',
  'penerapan': 'implementation', 'pengenaan': 'imposition', 'penghapusan': 'deletion',
  'pengirim': 'sender', 'penjelasan': 'explanation', 'penjumlahan': 'total', 'penyimpanan': 'storage',
  'perbaiki': 'fix', 'perkembangan': 'progress', 'pernah': 'ever', 'perpajakan': 'taxation',
  'persentase': 'percentage', 'pertambahan': 'increase', 'pihak': 'party', 'proteksi': 'protection',
  'prosedur': 'procedure', 'langkah': 'step', 'lanjutan': 'advanced', 'lapis': 'layer',
  'keinginan': 'desire', 'kendala': 'obstacle', 'ketersediaan': 'availability',
  'keberlangsungan': 'continuity', 'kurangnya': 'lack of', 'fondasi': 'foundation', 'netto': 'net',
  'aktiva': 'assets', 'pasiva': 'liabilities', 'setara': 'equivalent', 'tunai': 'cash', 'periode2': 'period',
  'tunggakan2': 'outstanding', 'arus kas': 'cash flow', 'laba rugi': 'profit & loss',
  'pengeluaran2': 'expense', 'penerimaan2': 'income', 'penyesuaian': 'adjustment', 'akrual': 'accrual',
  'disusutkan': 'depreciated', 'penyusutan': 'depreciation', 'revaluasi': 'revaluation', 'cadangan': 'reserve',

  // ── Kalimat deskripsi yang sering muncul (agar hasil utuh, bukan campur) ──
  'template (jenis dokumen)': 'Template (Document Type)',
  'template dipakai bersama, tetapi hasil ekstraksi, arsip & export hanya terlihat oleh pembuatnya (admin melihat semua)':
      'Templates are shared, but extraction results, archives & exports are only visible to their creator (admins can view all)',
  'template aktif akan dipakai untuk export pdf proforma invoice':
      'The active template will be used to export the Proforma Invoice PDF.',
  'gulir untuk melihat semua': 'scroll to view all',
  'ditandatangani': 'signed', 'tanda tangani': 'sign', 'kotak merah': 'red box',
  'menyimpan konfigurasi template untuk dipakai ulang': 'Save the template configuration to be reused.',
  'konfigurasi dapat dipakai ulang, tidak dibuat dari nol': 'The configuration can be reused — it is not built from scratch.',
  'desain dapat dipakai ulang, tidak dibuat ulang per dokumen': 'The design can be reused instead of being rebuilt for every document.',
  'mempercepat pembuatan variasi template': 'Speeds up creating template variations.',
  'untuk': 'to',
  'yang': 'that',
  'dengan': 'with',
  'dari': 'from',
  'atau': 'or',
  'pada': 'on',
  'saat': 'when',
  'baru': 'new',
  'ulang': 'again',
  'bukan': 'not',
  'perlu': 'need',
  'hanya': 'only',
  'masih': 'still',
  'terus': 'continue',
  'langsung': 'directly',
  'dulu': 'first',
  'sebab': 'because',
  'agar': 'so that',
  'supaya': 'so that',
  'kalau': 'if',
  'ketika': 'when',
  'tetapi': 'but',
  'tapi': 'but',
  'melalui': 'through',
  'terhadap': 'to',
  'tentang': 'about',
  'bagi': 'for',
  'oleh': 'by',
  'sebagai': 'as',
  'setiap': 'each',
  'masing': 'each',
  'sendiri': 'own',
  'lebih': 'more',
  'kurang': 'less',
  'sangat': 'very',
  'paling': 'most',
  'besar': 'large',
  'kecil': 'small',
  'siap': 'ready',
  'surat': 'letter',
  'nama': 'name',
  'alamat': 'address',
  'telepon': 'phone',
  'nanti': 'later',
  'kirimkan': 'send',
  'baris': 'row',
  'tabel': 'table',
  'bayaran': 'payment',
  'nota': 'memo',
  'kwitansi': 'receipt',
  'retur': 'return',
  'keluaran': 'output',
  'rekap': 'recap',
  'ringkas': 'concise',
  'utang': 'debt',
  'pendukung': 'supporting',
  'terlampir': 'attached',
  'buka': 'open',
  'pilihan': 'choice',
  'umum': 'general',
  'ringan': 'lightweight',
  'berbahaya': 'dangerous',
  'rusak': 'damaged',
  'hilang': 'lost',
  'konversi': 'conversion',
  'terjemah': 'translate',
  'inggris': 'English',
  'indonesia': 'Indonesian',
  'sampel': 'sample',
  'objek': 'object',
  'kepatuhan': 'compliance',
  'asli': 'original',
  'penerima': 'recipient',
  'subjek': 'subject',
  'simulasi': 'simulation',
  'konfigurasi': 'configuration',
  'integrasi': 'integration',
  'sinkron': 'sync',
  'backup': 'backup',
  'restore': 'restore',
  'migrasi': 'migration',
  'kembalikan': 'restore',
  'digital': 'digital',
  'pengingat': 'reminder',
  'rencana': 'plan',
  'transfer': 'transfer',
  'kartu': 'card',
  'kredit': 'credit',
  'debit': 'debit',
  'saldo': 'balance',
  'penarikan': 'withdrawal',
  'setoran': 'deposit',
  'gaji': 'salary',
  'upah': 'wage',
  'honor': 'honorarium',
  'tunjangan': 'allowance',
  'atasan': 'superior',
  'bawahan': 'subordinate',
  'usaha': 'business',
  'badan': 'entity',
  'mudah': 'easy',
  'sulit': 'hard',
  'rumit': 'complex',
  'praktis': 'practical',
  'efisien': 'efficient',
  'efektif': 'effective',
  'produktif': 'productive',
  'produktivitas': 'productivity',
  'performa': 'performance',
  'risiko': 'risk',
  'arsipkan': 'archive',
  'indeks': 'index',
  'urutkan': 'sort',
  'perluas': 'expand',
  'tarik': 'pull',
  'letak': 'position',
  'tema': 'theme',
  'hitam': 'black',
  'biru': 'blue',
  'tombol': 'button',
  'tautan': 'link',
  'angka': 'number',
  'karakter': 'character',
  'kata': 'word',
  'kalimat': 'sentence',
  'ekstensi': 'extension',
  'berikut': 'following',
  'seterusnya': 'onwards',
  'tepat': 'exact',
  'desimal': 'decimal',
  'presisi': 'precision',
  'akurasi': 'accuracy',
  'terjadi': 'occurs',
  'ditemukan': 'found',
  'kadang': 'sometimes',
  'macam': 'kind',
  'dipakai': 'used',
  'dibuat': 'created',
  'dibaca': 'read',
  'terkirim': 'sent',
  'berhenti': 'stop',
  'jalankan': 'run',
  'segarkan': 'refresh',
  'kurangi': 'reduce',
  'menerima': 'receive',
  'mencari': 'search',
  'membuka': 'open',
  'menutup': 'close',
  'mengubah': 'change',
  'menambah': 'add',
  'mengurangi': 'reduce',
  'mencetak': 'print',
  'mengekspor': 'export',
  'mengimpor': 'import',
  'menyegarkan': 'refresh',
  'menyembunyikan': 'hide',
  'memindahkan': 'move',
  'menempelkan': 'paste',
  'membagikan': 'share',
  'mempublikasikan': 'publish',
  'memulihkan': 'restore',
  'mengembalikan': 'return',
  'melampirkan': 'attach',
  'meneruskan': 'forward',
  'mengalihkan': 'redirect',
  'menjadwalkan': 'schedule',
  'melewati': 'skip',
  'menyarankan': 'suggest',
  'merekomendasikan': 'recommend',
  'memeriksa': 'check',
  'mengecek': 'check',
  'menghubungi': 'contact',
  'mengirimkan': 'send',
  'mengikuti': 'follow',
  'memahami': 'understand',
  'menjelaskan': 'explain',
  'menulis': 'write',
  'mendukung': 'support',
  'menjaga': 'maintain',
  'memantau': 'monitor',
  'meninjau': 'review',
  'mengevaluasi': 'evaluate',
  'mengukur': 'measure',
  'membandingkan': 'compare',
  'memisahkan': 'separate',
  'menggabungkan': 'merge',
  'mengurutkan': 'sort',
  'menyaring': 'filter',
  'menyortir': 'sort',
  'belum ada data untuk dibandingkan': 'No data to compare yet',
  'terjadi kesalahan saat menghapus data': 'An error occurred while deleting the data',
  'anda tidak memiliki izin untuk menghapus data': 'You do not have permission to delete data',
  'subjek email wajib diisi': 'Email subject is required',
  'lampiran proforma ditambahkan': 'Proforma attachment added',
  'export excel berhasil': 'Excel export succeeded',
  'invoice dihapus permanen': 'Invoice permanently deleted',
  'invoice dibatalkan': 'Invoice cancelled',
  'invoice diperbarui': 'Invoice updated',
  'invoice dipindah ke sampah': 'Invoice moved to trash',
  'proforma disetujui': 'Proforma approved',
  'proforma diajukan, menunggu approval accounting': 'Proforma submitted, awaiting accounting approval',
  'template dikembalikan ke default': 'Template restored to default',
  'berhasil memperbarui objek pajak': 'Tax object updated successfully',
  'dokumen sedang diproses ocr': 'document is being processed by OCR',
  'gagal mengambil detail po dp': 'Failed to fetch PO DP details',
  'gagal membuat pdf': 'Failed to generate PDF',
  'gagal mengunduh': 'Failed to download',
  'gagal mengekstrak dokumen': 'Failed to extract document',
  'gagal membaca baris sampel': 'Failed to read the sample row',
  'gagal menyalin dokumen': 'Failed to copy document',
  'gagal menyimpan berbagi': 'Failed to save sharing',
  'gagal menandai notifikasi dibaca': 'Failed to mark notification as read',
  'gagal download template': 'Failed to download template',
  'berhasil menambah objek pajak': 'Tax object added successfully',
  'tanda tangan pdf': 'PDF signature',
  'semua user': 'All users',
  'saran kerja hari ini': 'Today\u2019s work suggestions',
  'kotak sedang dipinjam': 'Box currently borrowed',
  'dokumen terbaru': 'Recent documents',
  'sedang diproses': 'Being processed',
  'sedang mengimport ratusan data wajib pajak ke sistem': 'Importing hundreds of taxpayer records into the system',
  'surat perintah pemeriksaan disampaikan kepada wajib pajak': 'The tax examination order is delivered to the taxpayer',
  'salin total pph periode b': 'Copy Period B PPh total',
  'isi alur dengan contoh default': 'Fill the flow with the default example',
  'isi minimal satu baris invoice asli sebelum menyimpan draft': 'Add at least one original invoice row before saving the draft',
  'minimal satu barang dengan model terisi': 'At least one item with a filled model is required',
  'qty semua barang harus lebih dari 0': 'Quantity of all items must be greater than 0',
  'input nilai penuh dan dp; sisanya dihitung otomatis': 'Enter the full amount and DP; the rest is calculated automatically',
  'uang masuk sebagai dp, pelunasan menyusul': 'Payment received as DP, settlement to follow',
  'barang dikirim dulu, pembayaran menyusul': 'Goods shipped first, payment to follow',
  'faktur pajak diajukan ke bagian tax': 'Tax invoice submitted to the tax division',
  'proforma perlu faktur pajak, segera lampirkan': 'Proforma needs a tax invoice, attach it soon',
  'proforma siap input pajak': 'Proforma ready for tax entry',
  'masukkan nama tipe pajak baru': 'Enter the new tax type name',
  'berhasil menjadikan revisi': 'Successfully set as revision',
  'jadikan file lampiran ini sebagai revisi terbaru dokumen': 'Make this attachment the latest document revision',
  'review log aktivitas hari ini untuk memastikan semua aman': 'Review today\u2019s activity log to make sure everything is safe',
  'waktunya merapikan struktur folder agar lebih efisien': 'Time to tidy up your folder structure for efficiency',
  'cek kapasitas gudang, mungkin ada slot yang bisa dioptimalkan': 'Check warehouse capacity, some slots may be optimizable',
  'jangan lupa periksa status audit pajak yang sedang berjalan': 'Don\u2019t forget to check the status of ongoing tax audits',
  'akses cepat file baru tanpa keluar dari dashboard': 'Quick access to new files without leaving the dashboard',
  'bahasa ocr belum tersedia, muat ulang halaman atau hubungi admin': 'OCR language not available, reload the page or contact the admin',
  'perkecil ukuran file pdf, 3 tingkat kualitas': 'Reduce PDF file size, 3 quality levels',
  'ekstrak teks dari pdf hasil scan atau gambar, 120+ bahasa didukung': 'Extract text from scanned/image PDFs, 120+ languages supported',
  'konversi berbagai format dokumen menjadi markdown': 'Convert various document formats to Markdown',
  'konversi pdf ke docx dengan tata letak asli': 'Convert PDF to DOCX with original layout',
  'pdf word konversi kompres gabung pecah ocr unlock merge split': 'PDF to Word, convert, compress, merge, split, OCR, unlock',
  'aktifkan untuk menempelkan ttd digital di atas garis shogo date pada pdf': 'Enable to place the digital signature above the SHOGO DATE line on the PDF',
  'pengetahuan tidak lagi bergantung pada individu tertentu': 'Knowledge no longer depends on any single individual',
  'monitoring penyelesaian kendala kerja': 'Monitoring of work issue resolution',
  'mengurangi waktu pencarian dokumen fisik di gudang': 'Reduces time spent finding physical documents in the warehouse',
  'kendala muncul saat karyawan cuti, mutasi, atau resign karena kurangnya panduan terdokumentasi': 'Issues arise when employees are on leave, transfer, or resign due to a lack of documented guides',
  'kelola master dealer atau barang': 'Manage master dealer/item',
  'secara permanen': 'permanently',
  'secara penuh': 'fully',
  'secara otomatis': 'automatically',
  'secara manual': 'manually',
  'secara real-time': 'in real time',
  'secara berkala': 'periodically',
  'secara terpusat': 'centrally',
  'secara mendalam': 'in depth',
  'secara langsung': 'directly',
  'secara visual': 'visually',
  'secara terstruktur': 'in a structured way',
  'secara presisi': 'precisely',
  'secara personal': 'personally',
  'secara terus-menerus': 'continuously',
  'muat ulang': 'reload',
  'muat ulang halaman': 'reload the page',
  'hubungi admin': 'contact the admin',
  'berkala': 'periodic',
  'sistem': 'system',
  'perhitungan': 'calculations',
  'disinkronkan': 'synchronized',
  'telah': 'has been',
  'antar': 'between',
  'anomali': 'anomaly',
  'menghindari': 'avoid',
  'gunakan': 'use',
  'terdokumentasi': 'documented',
  'tertentu': 'certain',
  'individu': 'individual',
  'tidak lagi': 'no longer',
  'pada sistem': 'in the system',
  'dan': 'and',
  'ke': 'to',
  'di': 'in',
  'ini': 'this',
  'itu': 'that',
  'tersebut': 'that',
  'adalah': 'is',
  'ialah': 'is',
  'yakni': 'namely',
  'yaitu': 'namely',
  'menjadi': 'become',
  'fitur': 'feature',
  'serta': 'and',
  'seperti': 'like',
  'misal': 'example',
  'cara': 'way',
  'tahap': 'stage',
  'syarat': 'condition',
  'ketentuan': 'provision',
  'aturan': 'rules',
  'kebijakan': 'policy',
  'mekanisme': 'mechanism',
  'seluruhnya': 'entirely',
  'diantara': 'among',
  'berupa': 'such as',
  'mengenai': 'regarding',
  'optimal': 'optimal',
  'dll': 'etc.',
  'dst': 'etc.',
  'dan lain-lain': 'and so on',
  'dll2': 'etc.',
  'adalah2': 'are',
  'tidak akan': 'will not',
  'tidak pernah': 'never',
  'cukup': 'enough',
  'pasti': 'certainly',
  'mungkin': 'maybe',
  'selalu': 'always',
  'kadang-kadang': 'sometimes',
  'biasanya': 'usually',
  'jarang': 'rarely',
  'akhirnya': 'finally',
  'awalnya': 'initially',
  'kemudian': 'then',
  'pertama kali': 'first time',
  'berikutnya': 'following',
  'disini': 'here',
  'diatas': 'above',
  'dibawah': 'below',
  'disamping': 'beside',
  'didepan': 'in front',
  'dibelakang': 'behind',
  'didalam': 'inside',
  'diluar': 'outside',
  'sebelah': 'next to',
  'dalam hal': 'in terms of',
  'hal ini': 'this matter',
  'oleh karena itu': 'therefore',
  'dengan demikian': 'thus',
  'tidak hanya': 'not only',
  'selain itu': 'furthermore',
  'terlebih lagi': 'moreover',
  'di samping itu': 'in addition',
  'sebagai berikut': 'as follows',
  'sebagai contoh': 'for example',
  'dalam rangka': 'in order to',
  'dalam hal ini': 'in this case',
  'dengan kata lain': 'in other words',
  'secara umum': 'in general',
  'secara khusus': 'in particular',
  'secara keseluruhan': 'overall',
  'secara bertahap': 'gradually',
  'secara bertahap2': 'in stages',
  'dari waktu ke waktu': 'from time to time',
  'saat ini': 'currently',
  'saat itu': 'at that time',
  'pada saat': 'when',
  'pada waktu': 'at the time',
  'pada hari': 'on the day',
  'pada tanggal': 'on the date',
  'pada bulan': 'in the month',
  'pada tahun': 'in the year',
  'setiap hari': 'daily',
  'setiap bulan': 'monthly',
  'setiap tahun': 'annually',
  'mingguan': 'weekly',
  'bulanan': 'monthly',
  'tahunan': 'yearly',
  'harian': 'daily',
  'per hari': 'per day',
  'per bulan': 'per month',
  'per tahun': 'per year',
  'hari kerja': 'working day',
  'hari libur': 'holiday',
  'jam kerja': 'working hours',
  'jam istirahat': 'break time',
  'jam operasional': 'operating hours',
  'waktu kerja': 'working time',
  'waktu luang': 'free time',
  'waktu tunggu': 'waiting time',
  'waktu proses': 'processing time',
  'waktu tempuh': 'travel time',
  'masa kerja': 'years of service',
  'tanggal berlaku': 'effective date',
  'tanggal berakhir': 'end date',
  'tanggal mulai': 'start date',
  'tanggal jatuh tempo': 'due date',
  'tanggal pengajuan': 'submission date',
  'tanggal persetujuan': 'approval date',
  'tanggal pembayaran': 'payment date',
  'tanggal transaksi': 'transaction date',
  'tanggal pelunasan': 'settlement date',
  'tanggal invoice': 'invoice date',
  'tanggal faktur': 'invoice date',
  'no faktur': 'invoice no',
  'no nota': 'memo no',
  'no dokumen': 'document no',
  'no surat': 'letter no',
  'no telepon': 'phone no',
  'no hp': 'mobile no',
  'no rekening': 'account no',
  'no npwp': 'tax id no',
  'no nik': 'national id no',
  'nama file': 'file name',
  'nama folder': 'folder name',
  'nama dokumen': 'document name',
  'nama template': 'template name',
  'nama tipe': 'type name',
  'nama divisi': 'division name',
  'nama departemen': 'department name',
  'nama user': 'username',
  'nama akun': 'account name',
  'nama perusahaan': 'company name',
  'nama usaha': 'business name',
  'nama bank': 'bank name',
  'nama pemilik': 'owner name',
  'nama penerima': 'recipient name',
  'nama pengirim': 'sender name',
  'nama atasan': 'superior name',
  'nama karyawan': 'employee name',
  'nama pegawai': 'employee name',
  'nama barang': 'item name',
  'nama produk': 'product name',
  'nama dealer': 'dealer name',
  'nama vendor': 'vendor name',
  'nama pemasok': 'supplier name',
  'nama pelanggan': 'customer name',
  'nama pembeli': 'buyer name',
  'nama penjual': 'seller name',
  'nama distributor': 'distributor name',
  'nama sales': 'sales name',
  'nama marketing': 'marketing name',
  'nama akunting': 'accounting name',
  'nama admin': 'admin name',
  'nama role': 'role name',
  'nama modul': 'module name',
  'nama menu': 'menu name',
  'nama kategori': 'category name',
  'nama kolom': 'column name',
  'nama tabel': 'table name',
  'nama field': 'field name',
  'nama status': 'status name',
  'nama tipe dokumen': 'document type name',
  'nama jenis dokumen': 'document type name',
  'tipe dokumen': 'document type',
  'jenis dokumen': 'document type',
  'tipe file': 'file type',
  'jenis file': 'file type',
  'tipe data': 'data type',
  'jenis data': 'data type',
  'tipe pajak': 'tax type',
  'jenis pajak': 'tax type',
  'tipe transaksi': 'transaction type',
  'jenis transaksi': 'transaction type',
  'tipe pembayaran': 'payment type',
  'jenis pembayaran': 'payment type',
  'tipe invoice': 'invoice type',
  'jenis invoice': 'invoice type',
  'tipe proforma': 'proforma type',
  'jenis proforma': 'proforma type',
  'tipe user': 'user type',
  'jenis user': 'user type',
  'tipe akun': 'account type',
  'jenis akun': 'account type',
  'tipe entry': 'entry type',
  'jenis entry': 'entry type',
  'kategori dokumen': 'document category',
  'kategori file': 'file category',
  'kategori barang': 'item category',
  'kategori biaya': 'expense category',
  'kategori pengeluaran': 'expense category',
  'kategori pemasukan': 'income category',
  'kategori pajak': 'tax category',
  'kategori template': 'template category',
  'kategori user': 'user category',
  'kategori role': 'role category',
  'kode': 'code',
  'resmi': 'official',
  'objek pajak': 'tax object',
  'wajib pajak': 'taxpayer',
  'memuat daftar': 'load the list',
  'daftar kode': 'list of codes',
  'lebih cepat': 'faster',
  'kerja lebih cepat': 'work faster',
  'pekerjaan lebih cepat': 'work faster',
  'setorkan': 'pay in',
  'selisihnya': 'the difference',
  'kode objek pajak': 'tax object code',
  'daftar kode objek pajak resmi': 'list of official tax object codes',
  'agar pekerjaan lebih cepat': 'so the work is faster',
  'untuk memuat daftar': 'to load the list',
  'memuat daftar kode': 'load the code list',
  'kode objek pajak resmi': 'official tax object codes',
  'guna': 'use',
  'gunakan fitur': 'use the feature',
  'fitur ini': 'this feature',
  'untuk melihat': 'to view',
  'untuk memudahkan': 'to make it easier',
  'untuk menghindari': 'to avoid',
  'untuk memastikan': 'to ensure',
  'untuk mempercepat': 'to speed up',
  'untuk mengelola': 'to manage',
  'untuk mengatur': 'to arrange',
  'untuk mencari': 'to search',
  'untuk menambahkan': 'to add',
  'untuk mengubah': 'to change',
  'untuk menghapus': 'to delete',
  'untuk menyimpan': 'to save',
  'untuk mencetak': 'to print',
  'untuk mengekspor': 'to export',
  'untuk mengunduh': 'to download',
  'untuk mengunggah': 'to upload',
  'untuk membandingkan': 'to compare',
  'untuk memproses': 'to process',
  'untuk memulai': 'to start',
  'untuk menyelesaikan': 'to finish',
  'untuk memeriksa': 'to check',
  'untuk memantau': 'to monitor',
  'untuk mengecek': 'to verify',
  'untuk mengisi': 'to fill in',
  'untuk memilih': 'to select',
  'untuk mengirim': 'to send',
  'untuk menerima': 'to receive',
  'untuk membuka': 'to open',
  'untuk menutup': 'to close',
  'untuk melanjutkan': 'to continue',
  'untuk membatalkan': 'to cancel',
  'untuk login': 'to log in',
  'untuk masuk': 'to sign in',
  'untuk keluar': 'to log out',
  'alur': 'flow',
  'alur persetujuan': 'approval flow',
  'anda': 'you',
  'pembuatnya': 'its creator',
  'lain': 'other',
  'tidak dapat': 'cannot',
  'tidak bisa': 'cannot',
  'dapat dibatalkan': 'can be cancelled',
  'tidak dapat dibatalkan': 'cannot be undone',
  'yakin ingin': 'sure you want',
  'apakah anda yakin': 'are you sure',
  'data ini': 'this data',
  'file pendukung': 'supporting file',
  'berkas pendukung': 'supporting document',
  'lampiran pendukung': 'supporting attachment',
  'maks': 'max',
  'maks 5mb': 'max 5MB',
  'dokumen baru': 'new document',
  'dokumen ini': 'this document',
  'ke departemen lain': 'to another department',
  'sisa pembayaran': 'remaining payment',
  'setelah pelunasan ini': 'after this settlement',
  'laporan bulanan': 'monthly report',
  'template email': 'email template',
  'kirim email': 'send email',
  'mengirim email': 'sending email',
  'ke 3 penerima': 'to 3 recipients',
  'email ke': 'email to',
  'tambah dokumen': 'add document',
  'ubah dokumen': 'edit document',
  'hapus dokumen': 'delete document',
  'detail dokumen': 'document details',
  'dokumen terpilih': 'selected document',
  'berbagi dokumen': 'share document',
  'bagikan dokumen': 'share the document',
  'bagikan ke': 'share to',
  'lihat dokumen': 'view document',
  'unduh dokumen': 'download document',
  'cetak dokumen': 'print document',
  'arsipkan dokumen': 'archive document',
  'pulihkan dokumen': 'restore document',
  'sampah dokumen': 'document trash',
  'dokumen di sampah': 'documents in trash',
  'tidak ada dokumen': 'no documents',
  'belum ada dokumen': 'no documents yet',
  'semua dokumen': 'all documents',
  'dokumen saya': 'my documents',
  'dokumen bersama': 'shared documents',
  'dokumen pribadi': 'private documents',
  'dokumen rahasia': 'confidential documents',
  'dokumen aktif': 'active documents',
  'dokumen selesai': 'completed documents',
  'dokumen draft': 'draft documents',
  'dokumen disetujui': 'approved documents',
  'dokumen ditolak': 'rejected documents',
  'dokumen pending': 'pending documents',
  'dokumen diproses': 'documents in process',
  'dokumen selesai diproses': 'documents processed',
  'dokumen gagal': 'failed documents',
  'dokumen error': 'error documents',
  'status dokumen': 'document status',
  'judul dokumen': 'document title',
  'kode dokumen': 'document code',
  'jumlah dokumen': 'number of documents',
  'total dokumen': 'total documents',
  'folder baru': 'new folder',
  'buat folder': 'create folder',
  'hapus folder': 'delete folder',
  'rename folder': 'rename folder',
  'pindah folder': 'move folder',
  'pindah ke folder': 'move to folder',
  'folder induk': 'parent folder',
  'sub folder': 'subfolder',
  'anak folder': 'child folder',
  'struktur folder': 'folder structure',
  'daftar folder': 'list of folders',
  'semua folder': 'all folders',
  'tambah dokumen baru': 'Add New Document',
  'hapus alur persetujuan ini': 'Delete this approval flow',
  'anda yakin ingin menghapus data ini secara permanen': 'Are you sure you want to permanently delete this data',
  'tindakan ini tidak dapat dibatalkan': 'This action cannot be undone',
  'dokumen ini hanya bisa dilihat oleh pembuatnya': 'This document can only be viewed by its creator',
  'bagikan ke departemen lain': 'Share to another department',
  'sisa pembayaran setelah pelunasan ini': 'Remaining payment after this settlement',
  'buat dokumen baru': 'Create New Document',
  'unggah dokumen baru': 'Upload New Document',
  'edit dokumen': 'Edit Document',
  'lihat detail': 'View Details',
  'lihat detail dokumen': 'View Document Details',
  'unduh dokumen ini': 'Download this document',
  'cetak dokumen ini': 'Print this document',
  'bagikan dokumen ini': 'Share this document',
  'arsipkan dokumen ini': 'Archive this document',
  'kembalikan dokumen ini': 'Restore this document',
  'hapus permanen dokumen ini': 'Permanently delete this document',
  'pindahkan ke sampah': 'Move to trash',
  'pindahkan ke folder': 'Move to folder',
  'salin ke folder': 'Copy to folder',
  'pindahkan dokumen ini ke': 'Move this document to',
  'pilih file untuk diunggah': 'Select a file to upload',
  'file berhasil diunggah': 'File uploaded successfully',
  'file berhasil diunduh': 'File downloaded successfully',
  'dokumen berhasil disimpan': 'Document saved successfully',
  'dokumen berhasil dihapus': 'Document deleted successfully',
  'dokumen berhasil dipindahkan': 'Document moved successfully',
  'dokumen berhasil disalin': 'Document copied successfully',
  'dokumen berhasil diarsipkan': 'Document archived successfully',
  'dokumen berhasil dipulihkan': 'Document restored successfully',
  'template berhasil disimpan': 'Template saved successfully',
  'template berhasil dihapus': 'Template deleted successfully',
  'gagal menyimpan dokumen': 'Failed to save document',
  'gagal menghapus dokumen': 'Failed to delete document',
  'gagal memuat dokumen': 'Failed to load documents',
  'sedang memuat dokumen': 'Loading documents',
  'memuat dokumen': 'Loading documents',
  'tidak ada dokumen ditemukan': 'No documents found',
  'hasil tidak ditemukan': 'No results found',
  'pencarian tidak ditemukan': 'No search results',
  'tidak ada hasil untuk': 'No results for',
  'data tidak ditemukan': 'Data not found',
  'belum ada data': 'No data yet',
  'belum ada data untuk ditampilkan': 'No data to display',
  'tidak ada data untuk ditampilkan': 'No data to display',
  'semua data': 'All data',
  'seluruh data': 'All data',
  'tampilkan semua': 'Show all',
  'menampilkan semua': 'Showing all',
  'urutkan berdasarkan': 'Sort by',
  'kelompokkan berdasarkan': 'Group by',
  'saring berdasarkan': 'Filter by',
  'filter berdasarkan': 'Filter by',
  'pilih semua': 'Select all',
  'hapus pilihan': 'Clear selection',
  'batalkan pilihan': 'Clear selection',
  'tandai semua': 'Select all',
  'centang semua': 'Select all',
  'batal pilih': 'Select none',
  'next page': 'Next page',
  'previous page': 'Previous page',
  'halaman sebelumnya': 'Previous page',
  'halaman berikutnya': 'Next page',
  'halaman pertama': 'First page',
  'halaman terakhir': 'Last page',
  'menampilkan 1 sampai': 'Showing 1 to',
  'baris per halaman': 'rows per page',
  'tampilkan baris': 'Show rows',
  'menyiapkan': 'preparing',
  'menyiapkan modul aplikasi': 'Preparing application module',
  'menyiapkan preview': 'Preparing preview',
  'menyiapkan import': 'Preparing import',
  'menyiapkan upload': 'Preparing upload',
  'menyiapkan file': 'Preparing file',
  'menyiapkan dokumen': 'Preparing documents',
  'menyiapkan data': 'Preparing data',
  'mempersiapkan': 'preparing',
  'persiapan': 'preparation',
  'persiapan data': 'data preparation',
  'siapkan': 'prepare',
  'disiapkan': 'prepared',
  'kesiapan': 'readiness',
  'kesiapan data': 'data readiness',
  'diolah': 'processed',
  'menyediakan': 'provides',
  'menyediakan jejak audit lengkap': 'provides a complete audit trail',
  'menjamin': 'ensures',
  'validitas': 'validity',
  'mengotomatisasi': 'automates',
  'mengotomatiskan': 'automates',
  'menyempurnakan': 'refines',
  'menyempurnakan model ai': 'refines the AI model',
  'meningkatkan': 'improves',
  'meningkatkan akurasi': 'improves accuracy',
  'memudahkan': 'makes it easier',
  'mempercepat': 'speeds up',
  'mempercepat proses': 'speeds up the process',
  'terekstrak': 'extracted',
  'mempermudah': 'simplifies',
  'menjadikan': 'makes',
  'menjadikan revisi': 'set as revision',
  'dapat dicari': 'searchable',
  'kebutuhan': 'needs',
  'keputusan': 'decisions',
  'kualitas': 'quality',
  'operasional': 'operational',
  'konsisten': 'consistent',
  'kontrol': 'control',
  'terstruktur': 'structured',
  'berkualitas': 'high quality',
  'berulang': 'repetitive',
  'tersebar': 'scattered',
  'terpusat': 'centralized',
  'eksekusi': 'execution',
  'standarisasi': 'standardization',
  'digitalisasi': 'digitization',
  'evaluasi': 'evaluation',
  'validasi': 'validation',
  'verifikasi': 'verification',
  'referensi': 'reference',
  'kapasitas': 'capacity',
  'pantau': 'monitor',
  'pemantauan': 'monitoring',
  'progres': 'progress',
  'membantu': 'helps',
  'mempercepat identifikasi': 'speeds up identification',
  'penyelesaian': 'resolution',
  'identifikasi': 'identification',
  'investigasi': 'investigation',
  'troubleshooting': 'troubleshooting',
  'keamanan sistem': 'system security',
  'bukti': 'evidence',
  'alat': 'tools',
  'satu alat': 'one tool',
  'keperluan': 'purposes',
  'berbagai keperluan': 'various purposes',
  'penggabungan': 'merging',
  'pemecahan': 'splitting',
  'pembukaan proteksi': 'unlocking',
  'buka proteksi': 'unlock protection',
  'kualitas terjaga': 'maintained quality',
  'terjaga': 'preserved',
  'dapat dicari dan diproses': 'searchable and processable',
  'dokumen scan': 'scanned documents',
  'hasil scan': 'scan results',
  'scan atau gambar': 'scan or image',
  'hasil scan atau gambar': 'scan or image results',
  'pengolahan': 'processing',
  'pengolahan pdf': 'PDF processing',
  'tanpa software': 'without software',
  'pihak ketiga': 'third party',
  'diolah ai': 'processed by AI',
  'pipeline ai': 'AI pipeline',
  'pencarian semantic': 'semantic search',
  'pratinjau hasil': 'result preview',
  'arsip digital': 'digital archive',
  'digitalisasi arsip': 'archive digitization',
  'konten non-digital': 'non-digital content',
  'input data': 'data entry',
  'input data manual': 'manual data entry',
  'rentan salah': 'prone to error',
  'salah hitung': 'miscalculation',
  'verifikasi dokumen': 'document verification',
  'alur kerja dokumen': 'document workflow',
  'mengurangi input data manual': 'reduces manual data entry',
  'ekstrak faktur': 'extract invoice',
  'ekstrak invoice': 'extract invoice',
  'latih template': 'train the template',
  'latih model': 'train the model',
  'data contoh': 'sample data',
  'data terekstrak': 'extracted data',
  'perbaiki model': 'improve the model',
  'mengurangi risiko': 'reduces risk',
  'kurangnya panduan': 'lack of guidance',
  'dokumen fisik': 'physical documents',
  'menemukan': 'find',
  'pencarian dokumen': 'document search',
  'kontrol kapasitas': 'capacity control',
  'percepat navigasi': 'speed up navigation',
  'pemantauan kpi': 'KPI monitoring',
  'real-time': 'real-time',
  'keputusan penting': 'important decisions',
  'tindak lanjut': 'follow-up',
  'human error': 'human error',
  'spreadsheet': 'spreadsheet',
  'rumus': 'formula',
  'berkurang': 'reduced',
  'perhitungan otomatis': 'automatic calculation',
  'kesalahan perhitungan': 'calculation errors',
  'otomatisasi': 'automation',
  'otomatisasi alur': 'workflow automation',
  'manual2': 'manually',
  'proses manual': 'manual process',
  'efisiensi': 'efficiency',
  'efisien2': 'efficient',
  'hemat waktu': 'saves time',
  'efektivitas': 'effectiveness',
  'kualitas data': 'data quality',
  'keandalan': 'reliability',
  'tersimpan aman': 'safely stored',
  'dilindungi': 'protected',
  'kehilangan data': 'data loss',
  'waktu henti': 'downtime',
  'kepatuhan pajak': 'tax compliance',
  'pelaporan pajak': 'tax reporting',
  'e-bupot': 'e-Bupot',
  'kesalahan pelaporan': 'reporting errors',
  'npwp': 'NPWP',
  'nomor npwp': 'tax identification number',
  'nik': 'ID number',
  'objek pajak baru': 'new tax object',
  'daftar kode objek': 'object code list',
  'menghindari kesalahan': 'avoid errors',
  'seiring waktu': 'over time',
  'meningkatkan akurasi model': 'improves model accuracy',
  'model semakin akurat': 'the model becomes more accurate',
  'mengekstrak informasi': 'extracting information',
  'contoh data': 'data examples',
  'dokumen contoh': 'sample documents',
};

// Peta lookup lowercase → translation (value disimpan persis untuk matchCase)
const EN_BY_ID = Object.create(null);
for (const [id, en] of Object.entries(ID2EN)) {
  EN_BY_ID[id.toLowerCase()] = en;
}
const PHRASES = Object.keys(EN_BY_ID).sort((a, b) => b.length - a.length);
const LETTER = /[A-Za-z\u00C0-\u024F]/;

function matchCase(original, translation) {
  if (!original || !translation) return translation;
  const first = original[0];
  let out = translation;
  if (/[A-Z]/.test(first)) {
    if (original.length >= 2 && original === original.toUpperCase()) out = out.toUpperCase();
    else out = out.charAt(0).toUpperCase() + out.slice(1);
  }
  // normalisasi mata uang "rp" -> "Rp"
  if (/\brp\b/i.test(out)) out = out.replace(/(^|\s)rp(?=\s|$)/gi, '$1Rp');
  return out;
}

/** Terjemahkan teks Indonesia → Inggris (panjang frasa diutamakan). */
export function translateIdToEn(input) {
  if (!input || typeof input !== 'string') return input;
  let out = '';
  let i = 0;
  const len = input.length;
  while (i < len) {
    const ch = input[i];
    if (!LETTER.test(ch)) { out += ch; i += 1; continue; }
    let matched = false;
    const rest = input.slice(i);
    const restLower = rest.toLowerCase();
    for (const phrase of PHRASES) {
      if (!restLower.startsWith(phrase)) continue;
      const end = i + phrase.length;
      const beforeOk = i === 0 || !LETTER.test(input[i - 1]);
      const afterOk = end >= len || !LETTER.test(input[end]);
      if (!beforeOk || !afterOk) continue;
      out += matchCase(input.slice(i, end), EN_BY_ID[phrase]);
      i = end;
      matched = true;
      break;
    }
    if (!matched) { out += ch; i += 1; }
  }
  return out;
}

// Cek cepat apakah teks mengandung kata Indonesia (hindari proses teks EN murni)
const INDO_WORD_STEMS = new Set(Object.keys(ID2EN).flatMap((p) => p.toLowerCase().split(' ')));
const WORD_SPLIT = /[^a-z\u00C0-\u024F]+/;

export function containsIndonesian(text) {
  if (!text || text.length > 4000) return false;
  const toks = text.toLowerCase().split(WORD_SPLIT);
  for (const tk of toks) if (tk.length > 1 && INDO_WORD_STEMS.has(tk)) return true;
  return false;
}
