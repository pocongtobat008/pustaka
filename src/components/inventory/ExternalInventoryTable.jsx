        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="bg-white dark:bg-[#0d0d0d] rounded-xl border border-stone-200 dark:border-white/[0.06] overflow-hidden">
                <table className="w-full text-sm text-left border-collapse">
                    <thead className="bg-white/60 dark:bg-[#0d0d0d]/60 text-stone-600 dark:text-white/70 backdrop-blur-xl border-b border-white/30 dark:border-white/5">
                        <tr>
                    ...
                    <tbody>
                        {externalItems.filter(isMatch).length === 0 ? (
                            <tr>
                                <td colSpan={7} className="px-6 py-8 text-center text-stone-500 dark:text-white/40">
                                    Belum ada data barang keluar {externalItems.length > 0 && "yang cocok dengan pencarian"}.
                                </td>
                            </tr>
                        ) : (
                            externalItems.filter(isMatch).map(item => (
                                <tr key={item.id} className="hover:bg-white/40 dark:hover:bg-white/[0.05]/40 transition-colors group border-b border-blue-50 dark:border-white/[0.06]/50">
                                    <td className="px-6 py-4 font-bold text-stone-800 dark:text-white flex items-center gap-3">
                                    ...
                                    <td className="px-6 py-4 text-stone-500 dark:text-white/40">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2 text-stone-600 dark:text-white/70">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-xs text-stone-500 dark:text-white/40 font-mono">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex gap-2 justify-end transition-opacity">
                                            <button
                                                ...
                                                className="group/btn relative p-2.5 bg-white dark:bg-[#0d0d0d] border border-stone-200 dark:border-white/[0.06] rounded-xl hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10 transition-all duration-300"
                                            ...
                                            >
                                                ...
                                                <FileText size={18} className="text-stone-400 group-hover/btn:text-blue-600 transition-colors duration-300 relative z-10" />
                                            </button>
                                            {hasPermission('inventory', 'edit') && (
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); onRestoreExternal(item); }}
                                                    className="group/btn relative p-2.5 bg-white dark:bg-[#0d0d0d] border border-stone-200 dark:border-white/[0.06] rounded-xl hover:border-emerald-500/50 hover:shadow-lg hover:shadow-emerald-500/10 transition-all duration-300"
                                                ...
                                                >
                                                    ...
                                                    <Truck size={18} className="text-stone-400 group-hover/btn:text-emerald-600 transition-colors duration-300 relative z-10 transform rotate-180" />
                                                </button>
                                            )}
                                        </div>
                                    </td>
