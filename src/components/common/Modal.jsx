...
const Modal = ({ isOpen, onClose, title, children, size = 'max-w-4xl', noPadding = false }) => {
    ...
    const modalContent = (
        ...
        >
            <div
                className={`my-6 sm:my-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl border border-white/50 dark:border-white/10 w-full ${size} max-h-[94vh] rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-300 relative group flex flex-col overflow-hidden`}
            ...
            >
                ...
                <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shrink-0 z-10">
                    <div>
                ...
                <div className={`${noPadding ? 'p-0' : 'p-6'} overflow-y-auto custom-scrollbar flex-1 min-h-0 relative z-0`}>
                    {children}
                </div>
            </div>
        </div>
