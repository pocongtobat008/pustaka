...
const OCRProcessingLanes = () => {
    ...
    return (
        <motion.div
            ...
            className="fixed bottom-8 right-8 z-[999] w-80 bg-white/80 dark:bg-[#0d0d0d]/80 backdrop-blur-xl rounded-[2.5rem] border border-stone-200 dark:border-white/10 shadow-2xl shadow-blue-500/10 overflow-hidden"
        >
            {/* Header - Klik untuk Toggle */}
            <div 
                onClick={() => setIsCollapsed(!isCollapsed)}
                className="p-5 flex items-center justify-between cursor-pointer hover:bg-stone-50/50 dark:hover:bg-white/5 transition-colors"
            >
                ...
                <div className="p-2 bg-stone-100 dark:bg-white/5 rounded-xl text-stone-400">
                            ...
                            {ocrJobs.map((job) => (
                                <div key={job.id} className="space-y-2 p-3 rounded-2xl bg-white/70 dark:bg-[#0d0d0d]/60 backdrop-blur-xl border border-transparent hover:border-blue-500/20 transition-all">
                                    <div className="flex items-center justify-between gap-3">
                                        ...
                                        <span className="text-[10px] font-black text-blue-600">{job.progress || 0}%</span>
                                    </div>
