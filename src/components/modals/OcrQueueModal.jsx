...
export default function OcrQueueModal({ ocrStats, API_BASE, toast }) {
  return (
    <div className="space-y-6 pt-4 px-1 pb-4">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {(ocrStats?.activeJobs || []).length === 0 ? (
            <div className="text-center py-8 bg-stone-50 dark:bg-[#0d0d0d]/50 rounded-3xl border-2 border-dashed border-stone-200 dark:border-white/[0.06]">
              <p className="text-stone-400 font-bold">Tidak ada pekerjaan yang sedang berjalan.</p>
            </div>
          ) : (
            ocrStats.activeJobs.map(job => (
              <div key={job.id} className="bg-white dark:bg-[#0d0d0d] p-4 rounded-[2rem] border border-stone-100 dark:border-white/[0.06] shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div className="min-w-0">
                    <p className="text-xs font-black text-stone-800 dark:text-white truncate">{job.filename}</p>
                    <p className="text-[10px] text-stone-400 font-black uppercase mt-0.5">Job ID: {job.id}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-lg font-black text-blue-600 dark:text-blue-400">{job.progress || 0}%</span>
                  </div>
                </div>
                <div className="w-full bg-stone-100 dark:bg-[#0d0d0d] rounded-full h-3 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
      ...
      <div className="pt-6 border-t border-stone-100 dark:border-white/[0.06] flex justify-between items-center">
        <button
          onClick={async () => {
