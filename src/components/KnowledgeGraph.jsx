    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
                <span className="font-bold text-stone-500 dark:text-white/40">Filter:</span>
                {['all', 'category', 'training_doc', 'knowledge', 'correction', 'brain_memory'].map(f => (
                    <button key={f} onClick={() => setFilter(f)}
                        className={`px-2 py-1 rounded-md transition-colors ${filter === f ? 'bg-blue-600 text-white' : 'bg-stone-100 dark:bg-[#0d0d0d] text-stone-600 dark:text-white/70 hover:bg-stone-200 dark:hover:bg-white/[0.06]'}`}>
                        {f === 'all' ? 'Semua' : (TYPE_LABEL[f] || f)}
                    </button>
                ))}
                <div className="ml-auto flex gap-3 text-[11px] text-stone-500 dark:text-white/40">
            ...
            {/* Legend */}
            <div className="flex flex-wrap gap-3 text-[11px] text-stone-500 dark:text-white/40">
            ...
            <div ref={wrapRef} className="relative w-full rounded-xl border dark:border-white/[0.06]/50 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-[#0d0d0d] overflow-hidden">
                <canvas
                    ...
                    onWheel={onWheel}
                />
                <div className="absolute bottom-2 left-2 text-[10px] text-stone-400 dark:text-white/30 pointer-events-none">
                ...
                return (
                    <div className="text-xs bg-white dark:bg-[#0d0d0d] border dark:border-white/[0.06]/50 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-stone-100 dark:bg-[#0d0d0d] text-stone-500 dark:text-white/40">{TYPE_LABEL[n.type] || n.type}</span>
                        </div>
                        {n.category && <div className="text-stone-500 dark:text-white/40">Kategori: {n.category}</div>}
                        {n.status && <div className="text-stone-500 dark:text-white/40">Status: {n.status}</div>}
                        {n.meta?.usedInTraining !== undefined && <div className="text-stone-500 dark:text-white/40">Terpakai di training: {n.meta.usedInTraining ? 'Ya' : 'Belum'}</div>}
                        {n.meta?.repeatCount !== undefined && <div className="text-stone-500 dark:text-white/40">Ditanyakan: {n.meta.repeatCount}x</div>}
                        {n.meta?.confidence !== undefined && <div className="text-stone-500 dark:text-white/40">Confidence: {(n.meta.confidence * 100).toFixed(0)}%</div>}
                        {n.meta?.applied !== undefined && <div className="text-stone-500 dark:text-white/40">Diterapkan: {n.meta.applied ? 'Ya' : 'Belum'}</div>}
                        {n.meta?.severity !== undefined && <div className="text-stone-500 dark:text-white/40">Severity: {(n.meta.severity * 100).toFixed(0)}%</div>}
                    </div>
                );
            })()}
        </div>
